 ...
 * المصدر الوحيد لـ bizId هو BusinessService (يُضبط من الـ layout فوراً من المسار)،
 * مع احتياط من المسار عند فتح رابط مباشر قبل تشغيل الـ layout.
 */
@Component({ template: '' })
export abstract class BasePageComponent implements OnDestroy {
  ...
  private readBizId(): number {
    ...
    if (globalThis.window !== undefined) return getBizIdFromUrl(globalThis.window.location.pathname);
    return 0;
  }
