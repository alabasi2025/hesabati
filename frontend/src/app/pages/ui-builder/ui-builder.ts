...
const SOURCE_TYPES = [
  {
    ...
    label: 'جدول مباشر',
    icon: 'storage',
    desc: 'جلب بيانات من جدول في قاعدة البيانات',
  },
...
export class UiBuilderComponent extends BasePageComponent {
  ...
  async deletePage(page: UiPage) {
    const confirmed = await this.toast.confirm({
      title: 'حذف الصفحة',
      message: `هل تريد حذف "${page.title}"؟`,
      type: 'danger',
    });
    if (!confirmed) return;
    try {
  ...
  openAddComponent() {
    ...
    const nextY =
      this.pageComponents().length > 0
        ? Math.max(...this.pageComponents().map((c) => c.positionY + c.height))
        : 0;
  ...
  async deleteComponent(comp: UiComponent) {
    const confirmed = await this.toast.confirm({
      title: 'حذف المكون',
      message: `هل تريد حذف "${comp.title || comp.componentType}"؟`,
      type: 'danger',
    });
    if (!confirmed) return;
    try {
  ...
  async deleteDataSource(ds: UiDataSource) {
    const confirmed = await this.toast.confirm({
      title: 'حذف مصدر البيانات',
      message: `هل تريد حذف "${ds.name}"؟`,
      type: 'danger',
    });
    if (!confirmed) return;
    try {
  ...
  getComponentTypeInfo(type: string) {
    return (
      COMPONENT_TYPES.find((t) => t.value === type) || { label: type, icon: 'widgets', desc: '' }
    );
  }
