    ...
    try {
        ...
        if (fs.existsSync(coveragePath)) {
            try {
                ...
                existingData = { jsCoverage: [], testTraces: {}, workerIndex: workerIndex };
                if (process.env.DEBUG_TDAD) console.debug('[TDAD] Parse error reading coverage file:', parseErr?.message);
            }
        }
        ...
        console.error('[TDAD] Failed to write trace:', error);
    }
}
...
function saveTraceFile(testTitle, testFile, trace, testStatus, testDuration, testError) {
    try {
        ...
        if (workflowsMatch) {
            workflowPath = workflowsMatch[1].replaceAll('\\', '/');
        }
        ...
        if (process.env.DEBUG_TDAD) console.debug('[TDAD] Trace file save failed:', traceErr?.message);
    }
}
...
const test = base.extend({
    tdadTrace: [async ({ page }, use, testInfo) => {
        ...
        try {
            ...
            if (process.env.DEBUG_TDAD) console.debug('[TDAD] JS coverage not available:', coverageErr?.message);
        }
        ...
        page.on('request', (request) => {
            ...
            if (contentType.includes('multipart/form-data')) {
                ...
                try {
                    ...
                    if (process.env.DEBUG_TDAD) console.debug('[TDAD] Request body not JSON:', parseErr?.message);
                }
            }
        ...
        page.on('response', async (response) => {
            ...
            try {
                ...
                if (process.env.DEBUG_TDAD) console.debug('[TDAD] Response not JSON:', parseErr?.message);
            }
            ...
            if (!entry) {
                ...
                if (contentType.includes('multipart/form-data')) {
                    ...
                    try {
                        ...
                        entry.requestBody = request.postData();
                        if (process.env.DEBUG_TDAD && parseErr) console.debug('[TDAD] Request body not JSON:', parseErr.message);
                    }
                }
            }
        ...
        async function captureSnapshot() {
            try {
                ...
                if (process.env.DEBUG_TDAD) console.debug('[TDAD] Snapshot failed:', snapshotErr?.message);
            }
            return null;
        }
        ...
        async function captureScreenshot(testTitle, testFile) {
            try {
                ...
                if (process.env.DEBUG_TDAD) console.debug('[TDAD] Screenshot failed:', screenshotErr?.message);
                return null;
            }
        }
        ...
        if (coverageStarted) {
            try {
                ...
                if (process.env.DEBUG_TDAD) console.debug('[TDAD] Stop coverage failed:', stopErr?.message);
            }
        }
        ...
        try {
            ...
            if (process.env.DEBUG_TDAD) console.debug('[TDAD] Read coverage file failed:', readErr?.message);
        }
