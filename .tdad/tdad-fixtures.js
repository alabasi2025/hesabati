...
const coveragePath = path.join(
  coverageDir,
  `coverage-worker-${workerIndex}.json`,
);
  ...
  tdadTrace: [
    async ({ page }, use, testInfo) => {
      ...
      try {
        ...
        /* Coverage not available in non-Chromium */
      }
      ...
      async function captureScreenshot(testTitle, testFile) {
        try {
          let workflowPath = "";
          const workflowsMatch = testFile.match(
            /\.tdad[\\/]workflows[\\/](.+)[\\/][^\\/]+\.test\.js$/,
          );
          if (workflowsMatch) {
      ...
      function saveTraceFile(
        ...
        try {
          ...
          /* Silently fail - trace file is non-critical */
        }
      }
      ...
      const createRequestWrapper = () => {
        ...
        methods.forEach((method) => {
          wrapper[method] = async (url, options) => {
            ...
            // Use url as-is; Playwright resolves relative URLs when requesting
            const fullUrl = url;
      ...
      const traceRef = {
        addApiRequest: (req) =>
          writeTraceIncremental(testTitle, { apiRequest: req }),
        addConsoleLog: (log) =>
          writeTraceIncremental(testTitle, { consoleLog: log }),
        addPageError: (err) =>
          writeTraceIncremental(testTitle, { pageError: err }),
        setActionResult: (result) =>
          writeTraceIncremental(testTitle, { actionResult: result }),
      ...
      const screenshotPath = isApiTest
        ? null
        : await captureScreenshot(testTitle, testInfo.file);
