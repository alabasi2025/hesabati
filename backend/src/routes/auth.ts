...
authRoutes.post("/register", async (c) => {
  try {
    ...
    if (!username || !password || !fullName) {
      return c.json(
        { error: "اسم المستخدم وكلمة المرور والاسم الكامل مطلوبان" },
        400,
      );
    }
...
// === POST /refresh - تجديد الـ access token باستخدام refresh token ===
authRoutes.post("/refresh", async (c) => {
  try {
    const refreshToken = getCookie(c, "hesabati_refresh");
    if (!refreshToken) {
      return c.json(
        { error: "لا يوجد refresh token - يرجى تسجيل الدخول" },
        401,
      );
    }
    ...
    if (!user || !user.isActive) {
      clearAuthCookies(c);
      return c.json({ error: "الحساب غير موجود أو معطل" }, 401);
    }
    ...
    clearAuthCookies(c);
    return c.json({ error: "فشل تجديد الجلسة" }, 500);
  }
});

// === POST /logout - تسجيل الخروج وحذف الـ cookies ===
