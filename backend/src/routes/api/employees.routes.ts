    ...
    if (usePagination) {
      ...
      const rows = await db
        .select({
          ...
          departmentId: columnSupport.departmentId
            ? employees.departmentId
            : sql<number | null>`NULL`,
          jobTitleId: columnSupport.jobTitleId
...
employeesRoutes.post(
  ...
  safeHandler("إضافة موظف", async (c: AppContext) => {
    ...
    const departmentId =
      typeof data.departmentId === "number" ? data.departmentId : Number.NaN;
    if (!Number.isInteger(departmentId) || departmentId <= 0) {
      return c.json(
        { error: "تصنيف الموظف (departmentId) مطلوب لإنشاء الحساب المرتبط" },
        400,
      );
    }
    const [created] = await db.transaction(async (tx) => {
      const employeeSeq = await getNextEmployeeSequence(
        bizId,
        departmentId,
        tx,
      );
...
employeesRoutes.put(
  ...
  safeHandler("تعديل موظف", async (c: AppContext) => {
    ...
    if (!existing)
      return c.json({ error: "موظف غير موجود أو لا ينتمي لهذا العمل" }, 404);
    const body = await getBody(c);
    ...
    if (columnSupport.departmentId)
      updatePayload.departmentId = body?.departmentId ?? body?.department_id;
    if (columnSupport.jobTitleId)
      updatePayload.jobTitleId = body?.jobTitleId ?? body?.job_title_id;
      ...
      await tx
        ...
        .where(
          and(
            eq(accounts.businessId, bizId),
            eq(accounts.accountType, "employee"),
            eq(accounts.linkedEmployeeId, id),
          ),
        );
...
employeesRoutes.put(
  "/employees/:id",
  safeHandler("تعديل موظف (legacy)", async (c: AppContext) => {
    const columnSupport = await getEmployeeColumnSupport();
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الموظف غير صالح" }, 400);
    const [existing] = await db
...
employeesRoutes.delete(
  "/employees/:id",
  safeHandler("حذف موظف (legacy)", async (c: AppContext) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الموظف غير صالح" }, 400);
    const [existing] = await db
      .select()
