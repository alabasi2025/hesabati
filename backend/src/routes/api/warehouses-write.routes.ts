...
warehousesWriteRoutes.post(
  ...
  safeHandler("إضافة مخزن", async (c) => {
    ...
    if (!accountId)
      return c.json(
        { error: "يجب اختيار حساب مرتبط بالمخزن من دليل الحسابات" },
        400,
      );
    ...
    const existingWarehouses = await db
      ...
      .where(
        and(
          ...
          eq(warehouses.accountId, accountId),
        ),
      );
    ...
    const warehouseType = (
      ["main", "station", "sub", "custody"].includes(
        String(body.warehouseType ?? ""),
      )
        ? body.warehouseType
        : "sub"
    ) as "main" | "station" | "sub" | "custody";
    ...
    const [created] = await db.transaction(async (tx) => {
      const data = {
        ...
        code: warehouseCode,
        responsiblePerson:
          typeof body.responsiblePerson === "string"
...
warehousesWriteRoutes.put(
  ...
  safeHandler("تعديل مخزن", async (c) => {
    ...
    if (!existing)
      return c.json({ error: "مخزن غير موجود أو لا ينتمي لهذا العمل" }, 404);
    const body = (await getBody(c)) as Record<string, unknown>;
