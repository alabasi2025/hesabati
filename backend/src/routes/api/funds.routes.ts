...
fundsRoutes.post('/businesses/:bizId/funds', bizAuthMiddleware(), checkPermission('funds', 'create'), safeHandler('إضافة صندوق', async (c: AppContext) => {
  ...
  const subTypeRaw = body.subType;
  if (subTypeRaw != null) {
    const subTypeId = Number.parseInt(String(subTypeRaw), 10);
    if (!Number.isNaN(subTypeId)) {
      insertPayload.sequenceNumber = await getNextSequence(bizId, 'fund_in_type', subTypeId, 0);
      insertPayload.code = generateItemCode(TYPE_PREFIXES.fund || 'FND', insertPayload.sequenceNumber as number);
    }
  }
