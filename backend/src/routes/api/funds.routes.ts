...
import { funds, fundBalances, stations, currencies, fundTypes } from '../../db/schema/index.ts';
...
import { getNextSequence, TYPE_PREFIXES, generateItemCode } from '../../middleware/sequencing.ts';
...
fundsRoutes.post('/businesses/:bizId/funds', bizAuthMiddleware(), checkPermission('funds', 'create'), safeHandler('إضافة صندوق', async (c: AppContext) => {
  ...
  const [ft] = await db
    ...
    .from(fundTypes)
    .where(and(eq(fundTypes.businessId, bizId), eq(fundTypes.subTypeKey, fundTypeKey)))
    .limit(1);

  const typeRefId = ft?.id ?? FUND_TYPE_FALLBACK_IDS[fundTypeKey] ?? 0;
  const manualSeqRaw = body.sequenceNumber;
  const manualSeq = (typeof manualSeqRaw === 'number')
    ? manualSeqRaw
  ...
  if (typeRefId > 0) {
    if (manualSeq != null && Number.isInteger(manualSeq) && manualSeq > 0) {
      const existing = await db
        .select({ id: funds.id })
        .from(funds)
        .where(and(eq(funds.businessId, bizId), eq(funds.fundType, fundTypeKey as any), eq(funds.sequenceNumber, manualSeq)))
        .limit(1);
      if (existing.length > 0) return c.json({ error: 'رقم الصندوق مستخدم مسبقاً داخل نفس التصنيف' }, 400);

      insertPayload.sequenceNumber = manualSeq;
      insertPayload.code = generateItemCode(TYPE_PREFIXES.fund || 'FND', manualSeq);
      await ensureCounterAtLeast(bizId, 'fund_in_type', typeRefId, 0, manualSeq);
    } else {
