...
import {
  ...
  generateJournalEntryFullSequence,
  TYPE_PREFIXES,
  generateItemCode,
...
api.get(
  "/businesses/:bizId/employee-billing-accounts",
  bizAuthMiddleware(),
  safeHandler("جلب حسابات الفوترة", async (c) => {
    ...
    const rows = await db
      ...
      .orderBy(
        employeeBillingAccounts.stationId,
        employeeBillingAccounts.employeeId,
        employeeBillingAccounts.sortOrder,
      );

    let filtered = rows;
    if (stationId)
      filtered = filtered.filter(
        (r) => r.stationId === Number.parseInt(stationId),
      );
    if (employeeId)
      filtered = filtered.filter(
        (r) => r.employeeId === Number.parseInt(employeeId),
      );
...
api.post(
  ...
  safeHandler("إضافة حساب فوترة", async (c) => {
    ...
    const [created] = await db
      .insert(employeeBillingAccounts)
...
api.put(
  "/employee-billing-accounts/:id",
  safeHandler("تعديل حساب فوترة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف حساب الفوترة غير صالح" }, 400);
    const [rec] = await db
      .select({ employeeId: employeeBillingAccounts.employeeId })
      .from(employeeBillingAccounts)
      .where(eq(employeeBillingAccounts.id, id));
...
api.delete(
  "/employee-billing-accounts/:id",
  safeHandler("حذف حساب فوترة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف حساب الفوترة غير صالح" }, 400);
    const [rec] = await db
      .select({ employeeId: employeeBillingAccounts.employeeId })
    ...
    if (err) return err;
    await db
      .delete(employeeBillingAccounts)
      .where(eq(employeeBillingAccounts.id, id));
  ...
  safeHandler("جلب الحسابات", async (c) => {
    ...
    if (accountIds.length > 0) {
      ...
      allLinks = await db
        ...
        .where(
          and(
            eq(accountAllowedLinks.isActive, true),
            inArray(accountAllowedLinks.fromAccountId, accountIds),
          ),
        );
    }
    ...
    const billingAsAccounts = billingRows.map((b) => {
      const sysName =
        BILLING_SYSTEM_NAMES[b.billingSystem || ""] || b.billingSystem || "";
      const methodName =
        COLLECTION_METHOD_NAMES[b.collectionMethod || ""] ||
        b.collectionMethod ||
        "";
      return {
        id: b.id,
...
api.post(
  ...
  safeHandler("إضافة حساب", async (c) => {
        ...
        const [t] = await db
          ...
          .where(
            and(
              ...
              eq(fundTypes.subTypeKey, subTypeKey),
            ),
          )
          .limit(1);
        typeId = t?.id ?? null;
    ...
    const [created] = await db
      .insert(accounts)
      .values({ ...accountData, businessId: bizId })
      .returning();
...
api.put(
  ...
  safeHandler("تعديل حساب", async (c) => {
    ...
    if (allowedLinks !== undefined) {
      await db
        .delete(accountAllowedLinks)
        .where(eq(accountAllowedLinks.fromAccountId, id));
      if (allowedLinks && allowedLinks.length > 0) {
        for (const link of allowedLinks) {
          await db
            .insert(accountAllowedLinks)
            .values({
              fromAccountId: id,
              toAccountId: link.toAccountId,
              linkType: link.linkType,
...
api.delete(
  ...
  safeHandler("حذف حساب", async (c) => {
    ...
    await db
      .delete(accountAllowedLinks)
      .where(eq(accountAllowedLinks.toAccountId, id));
...
api.put(
  "/accounts/:id",
  safeHandler("تعديل حساب (legacy)", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الحساب غير صالح" }, 400);
...
api.delete(
  "/accounts/:id",
  safeHandler("حذف حساب (legacy)", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الحساب غير صالح" }, 400);
    const [account] = await db
      .select()
...
api.get(
  "/accounts/:id",
  safeHandler("جلب حساب بالمعرّف", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الحساب غير صالح" }, 400);
    const [account] = await db
      .select()
...
api.get(
  "/funds/:id",
  safeHandler("جلب صندوق بالمعرّف", async (c) => {
    const id = parseId(c.req.param("id"));
    ...
    const balRows = await db
      .select()
      .from(fundBalances)
      .where(eq(fundBalances.fundId, id));
...
api.get(
  "/accounts/:id/allowed-links",
  safeHandler("جلب روابط الحسابات", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الحساب غير صالح" }, 400);
    const [account] = await db
      .select()
...
api.post(
  "/account-links",
  safeHandler("إضافة رابط حساب", async (c) => {
    const body = normalizeBody(await c.req.json());
    ...
    const [account] = await db
      .select()
      .from(accounts)
      .where(eq(accounts.id, body.fromAccountId));
    const err = await requireResourceOwnership(c, account ?? null);
    if (err) return err;
    const [created] = await db
      .insert(accountAllowedLinks)
...
api.delete(
  "/account-links/:id",
  safeHandler("حذف رابط حساب", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الرابط غير صالح" }, 400);
    const [link] = await db
      .select()
      .from(accountAllowedLinks)
      .where(eq(accountAllowedLinks.id, id));
    if (!link) return c.json({ error: "رابط غير موجود" }, 404);
    const [account] = await db
      .select()
      .from(accounts)
      .where(eq(accounts.id, link.fromAccountId));
    const err = await requireResourceOwnership(c, account ?? null);
    if (err) return err;
...
api.get(
  "/accounts/:id/allowed-targets",
  safeHandler("جلب الحسابات المسموحة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الحساب غير صالح" }, 400);
    const [account] = await db
      .select()
    ...
    const links = await db
      ...
      .where(
        and(
          eq(accountAllowedLinks.fromAccountId, id),
          eq(accountAllowedLinks.linkType, type),
        ),
      );
    return c.json(links);
  }),
...
// ===================== السندات =====================
api.get(
  "/businesses/:bizId/vouchers",
  bizAuthMiddleware(),
  safeHandler("جلب السندات", async (c) => {
    const bizId = getBizId(c);
  ...
  safeHandler("إضافة سند", async (c) => {
    ...
    const amount =
      typeof voucherData.amount === "string"
        ? Number.parseFloat(voucherData.amount)
        : voucherData.amount;
    if (Number.isNaN(amount) || amount <= 0)
      return c.json({ error: "المبلغ يجب أن يكون رقماً موجباً" }, 400);

    // تحويل voucherDate من string إلى Date
    if (
      voucherData.voucherDate &&
      typeof voucherData.voucherDate === "string"
    ) {
    ...
    if (voucherData.operationTypeId) {
      const otRows = await db.execute(
        sql`SELECT * FROM operation_types WHERE id = ${voucherData.operationTypeId} AND business_id = ${bizId}`,
      );
      opType = getFirstRow<Record<string, unknown>>(otRows) ?? null;
      if (!opType) return c.json({ error: "نوع العملية غير موجود" }, 404);
      if (opType.is_active === false)
        return c.json({ error: "نوع العملية غير مفعّل" }, 400);
    }

    const vType =
      (opType?.voucher_type as any) || voucherData.voucherType || "receipt";
    const tplFundId = (opType?.source_fund_id as number | undefined) ?? null;
    const fromFundId =
      voucherData.fromFundId || (vType === "payment" ? tplFundId : null);
    const toFundId =
      voucherData.toFundId || (vType === "receipt" ? tplFundId : null);

    // === تحديد الحسابات (مدين / دائن) ===
    let tplAccountId =
      (opType?.source_account_id as number | undefined) ?? null;
    // دعم القوالب النقدية: إن كان المصدر صندوقاً بدون حساب خزينة، نستخدم حساب نقدي نظامي
    const tplPaymentMethod =
      (opType?.payment_method as string | undefined) ?? null;
    ...
    if (vType === "receipt") {
      ...
      if (!debitAccountId)
        return c.json(
          {
            error:
              "الحساب المستلِم (toAccountId) مطلوب أو حدده في القالب (sourceAccountId)",
          },
          400,
        );
      if (!creditAccountId)
        return c.json(
          { error: "الحساب المصروف منه (fromAccountId) مطلوب لسند القبض" },
          400,
        );
    } else if (vType === "payment") {
      if (!debitAccountId)
        return c.json(
          { error: "الحساب المستلِم (toAccountId) مطلوب لسند الصرف" },
          400,
        );
      ...
      if (!creditAccountId)
        return c.json(
          {
            error:
              "الخزينة المصروف منها مطلوبة (fromAccountId أو sourceAccountId في القالب)",
          },
          400,
        );
    } else {
      ...
      if (!debitAccountId)
        return c.json({ error: "الحساب المستهدف (toAccountId) مطلوب" }, 400);
    ...
    if (
      debitAccountId &&
      !(await verifyAccountOwnership(debitAccountId, bizId))
    ) {
      return c.json({ error: "الحساب المستهدف لا ينتمي لهذا العمل" }, 403);
    }
    if (
      creditAccountId &&
      !(await verifyAccountOwnership(creditAccountId, bizId))
    ) {
    ...
    if (
      voucherData.voucherType === "transfer" ||
      opType?.voucher_type === "transfer"
    ) {
      // التحقق من نوع الصناديق المرتبطة
      let fromFundType: string | null = null;
      let toFundType: string | null = null;
      if (voucherData.fromFundId) {
        const fromFundResult = await db.execute(
          sql`SELECT fund_type FROM funds WHERE id = ${voucherData.fromFundId} AND business_id = ${bizId}`,
        );
        const fromRow = getFirstRow<{ fund_type: string }>(fromFundResult);
        fromFundType = fromRow?.fund_type ?? null;
      }
      if (voucherData.toFundId) {
        const toFundResult = await db.execute(
          sql`SELECT fund_type FROM funds WHERE id = ${voucherData.toFundId} AND business_id = ${bizId}`,
        );
      ...
      if (fromFundType === "safe" && toFundType === "safe") {
        return c.json(
          {
            error:
              "\u0644\u0627 \u064a\u0645\u0643\u0646 \u0627\u0644\u062a\u062d\u0648\u064a\u0644 \u0628\u064a\u0646 \u062e\u0632\u0627\u0626\u0646 (safe). \u0627\u0633\u062a\u062e\u062f\u0645 \u0639\u0645\u0644\u064a\u0629 \u0633\u062d\u0628/\u0625\u064a\u062f\u0627\u0639 \u0628\u062f\u0644\u0627\u064b \u0645\u0646 \u0630\u0644\u0643",
          },
          400,
        );
      }
    ...
    try {
      const result = await postTransaction(bizId, userId ?? 0, {
        ...
        description:
          voucherData.description || (opType?.name as string | undefined) || "",
      ...
      try {
        wsService.notifyNewVoucher(bizId, result.voucher);
      } catch {
        /* optional: skip if notify fails */
      }
      return c.json(result.voucher, 201);
    } catch (err: unknown) {
      return c.json(
        { error: toErrorMessage(err) || "فشل في تنفيذ المعاملة" },
        400,
      );
    }
  }),
...
api.delete(
  "/businesses/:bizId/vouchers/:id",
  bizAuthMiddleware(),
  checkPermission("vouchers", "delete"),
  safeHandler("حذف سند", async (c) => {
    const bizId = getBizId(c);
    const userId = getUserId(c);
    const id = parseId(c.req.param("id"));
...
api.delete(
  "/vouchers/:id",
  safeHandler("حذف سند (legacy)", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف السند غير صالح" }, 400);
    const [existing] = await db
      .select()
...
// ===================== التحصيل اليومي =====================
api.get(
  "/businesses/:bizId/collections",
  bizAuthMiddleware(),
  safeHandler("جلب التحصيلات", async (c) => {
    const bizId = getBizId(c);
    const stationId = c.req.query("stationId");
    ...
    if (stationId)
      conditions.push(
        eq(dailyCollections.stationId, Number.parseInt(stationId)),
      );
...
api.get(
  "/collections/:id",
  safeHandler("جلب تفاصيل تحصيل", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف التحصيل غير صالح" }, 400);
    const [collection] = await db
      .select()
      .from(dailyCollections)
      .where(eq(dailyCollections.id, id));
    ...
    const details = await db
      ...
      .leftJoin(
        employeeBillingAccounts,
        eq(collectionDetails.billingAccountId, employeeBillingAccounts.id),
      )
...
api.post(
  "/businesses/:bizId/collections",
  bizAuthMiddleware(),
  safeHandler("إضافة تحصيل", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const { details, ...collectionData } = body;

    if (!collectionData.stationId)
      return c.json({ error: "معرّف المحطة مطلوب" }, 400);
    if (!collectionData.collectionDate)
      return c.json({ error: "تاريخ التحصيل مطلوب" }, 400);

    let total = 0;
    if (details)
      for (const d of details) total += Number.parseFloat(d.amount || "0");

    const [created] = await db
      .insert(dailyCollections)
      .values({
        ...collectionData,
        businessId: bizId,
  ...
  safeHandler("إضافة توريد", async (c) => {
    ...
    if (col) {
      const allDeliveries = await db
        .select()
        .from(deliveryRecords)
        .where(eq(deliveryRecords.collectionId, collectionId));
      const totalDelivered = allDeliveries.reduce(
      ...
      if (totalDelivered >= totalCollected) {
        await db
          ...
          .set({ isFullyDelivered: true, updatedAt: new Date() })
          .where(eq(dailyCollections.id, collectionId));
      }
    }
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب الشركاء", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
...
api.post(
  ...
  safeHandler("إضافة شريك", async (c) => {
    ...
    const [created] = await db
      ...
      .values({ ...(validation.data as any), businessId: bizId })
      .returning();
    return c.json(created, 201);
  }),
);
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب الموردين", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(suppliers)
...
api.post(
  ...
  safeHandler("إضافة مخزن", async (c) => {
    ...
    if (subTypeRaw != null && subTypeRaw !== "") {
      const parsed = Number.parseInt(String(subTypeRaw), 10);
      if (!Number.isNaN(parsed)) {
        typeId = parsed;
        data.subTypeId = typeId;
      } else {
        const [t] = await db
          ...
          .where(
            and(
              eq(warehouseTypes.businessId, bizId),
              eq(warehouseTypes.subTypeKey, String(subTypeRaw)),
            ),
          )
          .limit(1);
        typeId = t?.id ?? null;
...
api.post(
  "/businesses/:bizId/pending-accounts",
  bizAuthMiddleware(),
  safeHandler("إضافة حساب معلق", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
...
api.put(
  "/pending-accounts/:id",
  safeHandler("تعديل حساب معلق", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الحساب المعلق غير صالح" }, 400);
    const [pending] = await db
      .select()
      .from(pendingAccounts)
      .where(eq(pendingAccounts.id, id));
...
api.delete(
  "/pending-accounts/:id",
  safeHandler("حذف حساب معلق", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الحساب المعلق غير صالح" }, 400);
    const [pending] = await db
      .select()
...
// ===================== التصفيات =====================
api.get(
  "/businesses/:bizId/settlements",
  bizAuthMiddleware(),
  safeHandler("جلب التصفيات", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(reconciliations)
      .where(eq(reconciliations.businessId, bizId))
...
api.post(
  "/businesses/:bizId/settlements",
  bizAuthMiddleware(),
  safeHandler("إضافة تصفية", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
...
api.put(
  "/settlements/:id",
  safeHandler("تعديل تصفية", async (c) => {
    const id = parseId(c.req.param("id"));
...
// ===================== تصنيفات السندات =====================
api.get(
  "/businesses/:bizId/voucher-categories",
  bizAuthMiddleware(),
  safeHandler("جلب تصنيفات السندات", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(voucherCategories)
      .where(eq(voucherCategories.businessId, bizId))
      .orderBy(voucherCategories.type, voucherCategories.name);
    return c.json(rows);
  }),
);
...
api.get(
  "/businesses/:bizId/expense-categories",
  bizAuthMiddleware(),
  safeHandler("جلب تصنيفات المصروفات", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(expenseCategories)
      .where(eq(expenseCategories.businessId, bizId))
...
api.post(
  "/businesses/:bizId/expense-categories",
  bizAuthMiddleware(),
  safeHandler("إضافة تصنيف مصروفات", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const [created] = await db
      .insert(expenseCategories)
      .values({ ...body, businessId: bizId })
      .returning();
    return c.json(created, 201);
  }),
);
api.put(
  ...
  safeHandler("تعديل تصنيف مصروفات", async (c) => {
    const id = parseId(c.req.param("id"));
...
api.delete(
  "/expense-categories/:id",
  safeHandler("حذف تصنيف مصروفات", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب ميزانية المصروفات", async (c) => {
    const bizId = getBizId(c);
    ...
    let result = rows;
    if (month)
      result = result.filter((r: any) => r.month === Number.parseInt(month));
    if (year)
      result = result.filter((r: any) => r.year === Number.parseInt(year));
    return c.json(result);
  }),
);
api.post(
  "/businesses/:bizId/expense-budget",
  bizAuthMiddleware(),
  safeHandler("إضافة بند ميزانية", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const [created] = await db
...
api.put(
  "/expense-budget/:id",
  safeHandler("تعديل بند ميزانية", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    const [rec] = await db
      .select()
      .from(expenseBudget)
      .where(eq(expenseBudget.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
    if (err) return err;
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
      .update(expenseBudget)
      .set({
        name: body.name,
        stationId: body.stationId ?? null,
...
api.delete(
  "/expense-budget/:id",
  safeHandler("حذف بند ميزانية", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب سجلات الرواتب", async (c) => {
    const bizId = getBizId(c);
    const month = c.req.query("month");
    const year = c.req.query("year");
    const rows = await db
      ...
      .orderBy(
        desc(salaryRecords.year),
        desc(salaryRecords.month),
        employees.fullName,
      );
    let result = rows;
    if (month)
...
api.post(
  "/businesses/:bizId/salaries",
  bizAuthMiddleware(),
  safeHandler("إضافة سجل راتب", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
...
api.put(
  "/salaries/:id",
  safeHandler("تعديل سجل راتب", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.delete(
  "/salaries/:id",
  safeHandler("حذف سجل راتب", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.get(
  "/currencies",
  safeHandler("جلب العملات", async (c) => {
...
api.get(
  ...
  safeHandler("جلب المرفقات", async (c) => {
...
api.get(
  "/stations",
  safeHandler("جلب المحطات (legacy)", async (c) => {
...
api.get(
  "/employees",
  safeHandler("جلب الموظفين (legacy)", async (c) => {
    const rows = await db
      .select({
        id: employees.id,
...
api.get(
  "/accounts",
  safeHandler("جلب الحسابات (legacy)", async (c) => {
    const rows = await db
      .select()
      .from(accounts)
      .orderBy(accounts.accountType, accounts.name);
...
api.get(
  "/funds",
  safeHandler("جلب الصناديق (legacy)", async (c) => {
    const rows = await db
      .select()
      .from(funds)
      .orderBy(funds.fundType, funds.name);
...
api.get(
  "/voucher-categories",
  safeHandler("جلب تصنيفات السندات (legacy)", async (c) => {
    const rows = await db
      .select()
      .from(voucherCategories)
      .orderBy(voucherCategories.type, voucherCategories.name);
...
api.get(
  "/suppliers",
  safeHandler("جلب الموردين (legacy)", async (c) => {
...
api.get(
  "/businesses/:bizId/operation-types",
  bizAuthMiddleware(),
  safeHandler("جلب أنواع العمليات", async (c) => {
    const bizId = getBizId(c);
    ...
    if (screen)
      conditions.push(sql`${screen} = ANY(${operationTypes.screens})`);
    ...
    if (otIds.length > 0) {
      allLinkedAccounts = await db
        .select({
          ...
          employeeBillingAccountId:
            operationTypeAccounts.employeeBillingAccountId,
...
api.get(
  "/operation-types/:id",
  safeHandler("جلب تفاصيل نوع عملية", async (c) => {
    const id = parseId(c.req.param("id"));
    ...
    return c.json({
      ...ot!,
      linkedAccounts: linkedWithDisplay,
      accounts: linkedWithDisplay,
    });
  }),
);
...
api.post(
  "/businesses/:bizId/operation-types",
  bizAuthMiddleware(),
  safeHandler("إضافة نوع عملية", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    ...
    if (willBeActive && (vt === "receipt" || vt === "payment")) {
      ...
      if (!pm)
        return c.json({ error: "وسيلة الدفع مطلوبة لسندات القبض/الصرف" }, 400);
      if (pm === "cash") {
        if (!otData.sourceFundId)
          return c.json(
            { error: "اختر الخزينة (الصندوق) في القالب قبل الحفظ" },
            400,
          );
      } else {
        if (!otData.sourceAccountId)
          return c.json(
            { error: "اختر الخزينة (حساب بنك/صراف/محفظة) في القالب قبل الحفظ" },
            400,
          );
      }
      if (!Array.isArray(laList) || laList.length === 0) {
        return c.json(
          {
            error:
              "اختر حساباً واحداً على الأقل في الحسابات المرتبطة (الطرف الثاني) قبل الحفظ",
          },
          400,
        );
      }
    ...
    const [seqResult] = await db
      ...
      .where(
        and(
          ...
          eq(operationTypes.category, category),
        ),
      );
      ...
      await db.insert(operationTypeAccounts).values(
        laList.map((la: any, i: number) => {
          ...
          return {
            ...
            permission:
              typeof la === "object" ? la.permission || "both" : "both",
...
api.put(
  "/operation-types/:id",
  safeHandler("تعديل نوع عملية", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف نوع العملية غير صالح" }, 400);
    const [ot] = await db
      .select()
    ...
    // === إلزامية تحديد الخزينة قبل الحفظ (حسب نوع السند) ===
    const nextIsActive =
      (body.isActive !== undefined ? body.isActive : ot?.isActive) !== false;
    const nextVoucherType = String(
      body.voucherType ?? ot?.voucherType ?? "",
    ).trim();
    if (
      nextIsActive &&
      (nextVoucherType === "receipt" || nextVoucherType === "payment")
    ) {
      const nextPaymentMethod = String(
        body.paymentMethod ?? ot?.paymentMethod ?? "",
      ).trim();
      const nextSourceFundId =
        body.sourceFundId !== undefined ? body.sourceFundId : ot?.sourceFundId;
      const nextSourceAccountId =
        body.sourceAccountId !== undefined
...
api.delete(
  "/operation-types/:id",
  safeHandler("حذف نوع عملية", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف نوع العملية غير صالح" }, 400);
    const [ot] = await db
      .select()
...
api.post(
  "/operation-types/:otId/accounts",
  safeHandler("ربط حساب بنوع عملية", async (c) => {
    const otId = parseId(c.req.param("otId"));
    if (!otId) return c.json({ error: "معرّف نوع العملية غير صالح" }, 400);
    const [ot] = await db
      .select()
      .from(operationTypes)
      .where(eq(operationTypes.id, otId));
    const err = await requireResourceOwnership(c, ot ?? null);
    if (err) return err;
    const body = normalizeBody(await c.req.json());
    ...
    const [created] = await db
      .insert(operationTypeAccounts)
      .values({ ...body, operationTypeId: otId })
      .returning();
    return c.json(created, 201);
  }),
);
...
api.delete(
  "/operation-type-accounts/:id",
  safeHandler("فك ربط حساب من نوع عملية", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الربط غير صالح" }, 400);
    const [ota] = await db
      .select()
      .from(operationTypeAccounts)
      .where(eq(operationTypeAccounts.id, id));
    if (!ota) return c.json({ error: "ربط غير موجود" }, 404);
    const [ot] = await db
      .select()
      .from(operationTypes)
      .where(eq(operationTypes.id, ota.operationTypeId));
    const err = await requireResourceOwnership(c, ot ?? null);
    if (err) return err;
    await db
      .delete(operationTypeAccounts)
  ...
  safeHandler("إضافة قيد محاسبي", async (c) => {
    ...
    if (!entryData.operationTypeId) {
      return c.json(
        {
          error:
            "\u0645\u0639\u0631\u0651\u0641 \u0646\u0648\u0639 \u0627\u0644\u0639\u0645\u0644\u064a\u0629 (\u0627\u0644\u0642\u0627\u0644\u0628) \u0645\u0637\u0644\u0648\u0628 - operationTypeId",
        },
        400,
      );
    }
    ...
    const categoryKey = String(
      entryData.categoryKey || entryData.category || "",
    ).trim();
    if (!categoryKey)
      return c.json({ error: "تصنيف قيد اليومية مطلوب (categoryKey)" }, 400);
    ...
    if (!cat.sortOrder || cat.sortOrder <= 0)
      return c.json({ error: "رقم تصنيف القيد غير مضبوط (sortOrder)" }, 400);

    const [opType] = await db
      .select({ code: operationTypes.code, name: operationTypes.name })
      .from(operationTypes)
      .where(
        and(
          eq(operationTypes.id, entryData.operationTypeId),
          eq(operationTypes.businessId, bizId),
        ),
      )
      .limit(1);
    if (!opType)
      return c.json({ error: "نوع العملية (القالب) غير موجود" }, 400);
    ...
    let entryNumber = `${jPrefix}-${cat.sortOrder}-${tplCode}-${year}-${seqVal}`;
    ...
    if (entryData.operationTypeId) {
      ...
      if (opTypeRow) {
        ...
        entryNumber =
          entryData.reference || entryNumber || jeSeqResult.fullSequenceNumber;
        journalCategorySeq = String(catSeqNum);
        journalTemplateSeq = String(jeSeqResult.sequentialNumber);
      }
    }
    if (!journalFullSeqNum) journalFullSeqNum = entryNumber;
    ...
    const [entry] = await db
      ...
      .values({
        ...
        categorySequence:
          journalCategorySeq ?? `${cat.sortOrder}-${year}-${seqVal}`,
        templateSequence: journalTemplateSeq ?? `${tplCode}-${year}-${seqVal}`,
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب إعدادات أنظمة الفوترة", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
...
api.post(
  "/businesses/:bizId/billing-systems-config",
  bizAuthMiddleware(),
  safeHandler("إضافة إعداد نظام فوترة", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
...
api.put(
  ...
  safeHandler("تعديل إعداد نظام فوترة", async (c) => {
    const id = parseId(c.req.param("id"));
...
api.delete(
  "/billing-systems-config/:id",
  safeHandler("حذف إعداد نظام فوترة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الإعداد غير صالح" }, 400);
    const [rec] = await db
      .select()
...
// ===================== أنواع الصناديق =====================
api.get(
  "/businesses/:bizId/fund-types",
  bizAuthMiddleware(),
  safeHandler("جلب أنواع الصناديق", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(fundTypes)
      .where(eq(fundTypes.businessId, bizId))
      .orderBy(fundTypes.sortOrder);
    return c.json(rows);
  }),
);
...
api.post(
  "/businesses/:bizId/fund-types",
  bizAuthMiddleware(),
  safeHandler("إضافة نوع صندوق", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    ...
    if (existing.length > 0)
      return c.json({ error: "رمز التصنيف مستخدم مسبقاً" }, 400);
...
api.put(
  "/fund-types/:id",
  safeHandler("تعديل نوع صندوق", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
...
api.delete(
  "/fund-types/:id",
  safeHandler("حذف نوع صندوق", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
    const [rec] = await db.select().from(fundTypes).where(eq(fundTypes.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
...
// ===================== أنواع البنوك =====================
api.get(
  "/businesses/:bizId/bank-types",
  bizAuthMiddleware(),
  safeHandler("جلب أنواع البنوك", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(bankTypes)
      .where(eq(bankTypes.businessId, bizId))
      .orderBy(bankTypes.sortOrder);
    return c.json(rows);
  }),
);
...
api.post(
  "/businesses/:bizId/bank-types",
  bizAuthMiddleware(),
  safeHandler("إضافة نوع بنك", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const validation = validateBody(typeSchema, body);
    if (!validation.success) return c.json({ error: validation.error }, 400);
...
api.put(
  "/bank-types/:id",
  safeHandler("تعديل نوع بنك", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
...
api.delete(
  "/bank-types/:id",
  safeHandler("حذف نوع بنك", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
    const [rec] = await db.select().from(bankTypes).where(eq(bankTypes.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
...
// ===================== أنواع الصرافين =====================
api.get(
  "/businesses/:bizId/exchange-types",
  bizAuthMiddleware(),
  safeHandler("جلب أنواع الصرافين", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(exchangeTypes)
      .where(eq(exchangeTypes.businessId, bizId))
      .orderBy(exchangeTypes.sortOrder);
    return c.json(rows);
  }),
);
...
api.post(
  "/businesses/:bizId/exchange-types",
  bizAuthMiddleware(),
  safeHandler("إضافة نوع صراف", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const validation = validateBody(typeSchema, body);
    if (!validation.success) return c.json({ error: validation.error }, 400);
...
api.put(
  "/exchange-types/:id",
  safeHandler("تعديل نوع صراف", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
    const [rec] = await db
      .select()
      .from(exchangeTypes)
      .where(eq(exchangeTypes.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
    if (err) return err;
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
      .update(exchangeTypes)
      .set({ ...body, updatedAt: new Date() })
      .where(eq(exchangeTypes.id, id))
      .returning();
    if (!updated) return c.json({ error: "نوع غير موجود" }, 404);
...
api.delete(
  "/exchange-types/:id",
  safeHandler("حذف نوع صراف", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.get(
  "/businesses/:bizId/e-wallet-types",
  bizAuthMiddleware(),
  safeHandler("جلب أنواع المحافظ", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(eWalletTypes)
      .where(eq(eWalletTypes.businessId, bizId))
      .orderBy(eWalletTypes.sortOrder);
    return c.json(rows);
  }),
);
...
api.post(
  "/businesses/:bizId/e-wallet-types",
  bizAuthMiddleware(),
  safeHandler("إضافة نوع محفظة", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const validation = validateBody(typeSchema, body);
    if (!validation.success) return c.json({ error: validation.error }, 400);
...
api.put(
  "/e-wallet-types/:id",
  safeHandler("تعديل نوع محفظة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
    const [rec] = await db
      .select()
      .from(eWalletTypes)
      .where(eq(eWalletTypes.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
    if (err) return err;
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
...
api.delete(
  "/e-wallet-types/:id",
  safeHandler("حذف نوع محفظة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف النوع غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.get(
  ...
  safeHandler("جلب المستخدمين", async (c) => {
    ...
    if (!user || user.role !== "admin")
      return c.json({ error: "غير مصرح - فقط المدير" }, 403);
    const rows = await db
      .select({
        id: users.id,
...
// ===================== الشاشات المخصصة =====================
api.get(
  "/businesses/:bizId/screens",
  bizAuthMiddleware(),
  safeHandler("جلب الشاشات", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(screenTemplates)
...
api.post(
  "/businesses/:bizId/screens",
  bizAuthMiddleware(),
  safeHandler("إضافة شاشة", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
      ...
      try {
        ...
        if (sidebarSectionId) {
          ...
          const [section] = await db
            .select()
            .from(sidebarSections)
            .where(eq(sidebarSections.id, sidebarSectionId));
          if (section) {
            ...
            const [defaultSection] = await db
              ...
              .where(
                and(
                  eq(sidebarSections.businessId, bizId),
                  eq(sidebarSections.name, "شاشات مخصصة"),
                ),
              );
        ...
        for (const userId of userIds) {
          await db.insert(userSidebarConfig).values({
            ...
            isVisible: true,
            customSortOrder: sidebarItem.sortOrder || 0,
          });
        }
      } catch (sidebarErr: unknown) {
        console.error(
          "خطأ في إضافة الشاشة للسايدبار:",
          toErrorMessage(sidebarErr),
        );
...
api.put(
  "/screens/:id",
  safeHandler("تعديل شاشة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    const [rec] = await db
      .select()
      .from(screenTemplates)
      .where(eq(screenTemplates.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
    if (err) return err;
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
...
api.delete(
  ...
  safeHandler("حذف شاشة", async (c) => {
    ...
    for (const w of widgets) {
      await db
        .delete(screenWidgetTemplates)
        .where(eq(screenWidgetTemplates.widgetId, w.id));
      await db
        .delete(screenWidgetAccounts)
        .where(eq(screenWidgetAccounts.widgetId, w.id));
    }
    await db.delete(screenWidgets).where(eq(screenWidgets.screenId, id));
    await db
      .delete(screenPermissions)
      .where(eq(screenPermissions.screenId, id));
    // حذف عنصر السايدبار المرتبط بالشاشة (إن وجد)
    const sidebarRoute = `custom-screens?screen=${id}`;
    const linkedSidebarItems = await db
      .select()
      .from(sidebarItems)
      .where(sql`${sidebarItems.route} LIKE ${"%" + sidebarRoute}`);
...
// ===================== ويدجتس الشاشات =====================
api.get(
  "/screens/:screenId/widgets",
  safeHandler("جلب ويدجتس الشاشة", async (c) => {
...
api.post(
  "/screens/:screenId/widgets",
  safeHandler("إضافة ويدجت", async (c) => {
    const screenId = parseId(c.req.param("screenId"));
    if (screenId) {
      const [scr] = await db
        .select()
    ...
    const body = normalizeBody(await c.req.json());
    if (!body.widgetType)
      return c.json({ error: "نوع العنصر (widgetType) مطلوب" }, 400);
    if (!body.title)
      return c.json({ error: "عنوان العنصر (title) مطلوب" }, 400);
    const [created] = await db
      .insert(screenWidgets)
      .values({ ...body, screenId })
      .returning();
    return c.json(created, 201);
  }),
);
...
api.put(
  "/widgets/:id",
  safeHandler("تعديل ويدجت", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
...
api.delete(
  "/widgets/:id",
  safeHandler("حذف ويدجت", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    await db
      .delete(screenWidgetTemplates)
      .where(eq(screenWidgetTemplates.widgetId, id));
    await db
      .delete(screenWidgetAccounts)
      .where(eq(screenWidgetAccounts.widgetId, id));
...
api.put(
  "/screens/:screenId/widgets/batch",
  safeHandler("تحديث ويدجتس دفعة واحدة", async (c) => {
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    const { widgets } = body;
    if (!widgets || !Array.isArray(widgets))
      return c.json({ error: "قائمة العناصر (widgets) مطلوبة" }, 400);
    ...
    for (const w of widgets) {
      if (w.id) {
        const [updated] = await db
          ...
          .where(eq(screenWidgets.id, w.id))
          .returning();
...
api.get(
  "/widgets/:widgetId/templates",
  safeHandler("جلب قوالب الويدجت", async (c) => {
    const widgetId = parseId(c.req.param("widgetId"));
    if (!widgetId) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    const rows = await db
      .select({
        id: screenWidgetTemplates.id,
        widgetId: screenWidgetTemplates.widgetId,
        operationTypeId: screenWidgetTemplates.operationTypeId,
        sortOrder: screenWidgetTemplates.sortOrder,
        operationName: operationTypes.name,
        operationIcon: operationTypes.icon,
        operationColor: operationTypes.color,
      })
      .from(screenWidgetTemplates)
      .leftJoin(
        operationTypes,
        eq(screenWidgetTemplates.operationTypeId, operationTypes.id),
      )
...
api.post(
  "/widgets/:widgetId/templates",
  safeHandler("ربط قالب بويدجت", async (c) => {
    const widgetId = parseId(c.req.param("widgetId"));
    if (!widgetId) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    if (!body.operationTypeId)
      return c.json({ error: "معرّف نوع العملية مطلوب" }, 400);
    const [created] = await db
      .insert(screenWidgetTemplates)
      .values({ ...body, widgetId })
      .returning();
    return c.json(created, 201);
  }),
);
...
api.delete(
  "/widget-templates/:id",
  safeHandler("فك ربط قالب من ويدجت", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الربط غير صالح" }, 400);
    await db
      .delete(screenWidgetTemplates)
      .where(eq(screenWidgetTemplates.id, id));
    return c.json({ success: true });
  }),
);

api.get(
  "/widgets/:widgetId/accounts",
  safeHandler("جلب حسابات الويدجت", async (c) => {
    const widgetId = parseId(c.req.param("widgetId"));
    if (!widgetId) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    const rows = await db
      .select({
        id: screenWidgetAccounts.id,
        widgetId: screenWidgetAccounts.widgetId,
        accountId: screenWidgetAccounts.accountId,
        sortOrder: screenWidgetAccounts.sortOrder,
        accountName: accounts.name,
        accountType: accounts.accountType,
      })
...
api.post(
  "/widgets/:widgetId/accounts",
  safeHandler("ربط حساب بويدجت", async (c) => {
    const widgetId = parseId(c.req.param("widgetId"));
    if (!widgetId) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    if (!body.accountId) return c.json({ error: "معرّف الحساب مطلوب" }, 400);
    const [created] = await db
      .insert(screenWidgetAccounts)
      .values({ ...body, widgetId })
      .returning();
    return c.json(created, 201);
  }),
...
api.delete(
  "/widget-accounts/:id",
  safeHandler("فك ربط حساب من ويدجت", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الربط غير صالح" }, 400);
    await db
      .delete(screenWidgetAccounts)
      .where(eq(screenWidgetAccounts.id, id));
    return c.json({ success: true });
  }),
);
...
// ===================== نسخ الشاشات =====================
api.post(
  "/screens/:screenId/clone",
  safeHandler("نسخ شاشة", async (c) => {
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
...
api.post(
  "/widgets/:widgetId/copy-to/:targetScreenId",
  safeHandler("نسخ ويدجت لشاشة أخرى", async (c) => {
    const widgetId = parseId(c.req.param("widgetId"));
    const targetScreenId = parseId(c.req.param("targetScreenId"));
    if (!widgetId) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    if (!targetScreenId)
      return c.json({ error: "معرّف الشاشة الهدف غير صالح" }, 400);

    const [original] = await db
      .select()
      .from(screenWidgets)
      .where(eq(screenWidgets.id, widgetId));
...
api.get(
  "/businesses/:bizId/screens-with-widgets",
  bizAuthMiddleware(),
  safeHandler("جلب الشاشات مع الويدجتس", async (c) => {
    const bizId = getBizId(c);
    ...
    return c.json(
      screensArr.map((s) => ({ ...s, widgets: widgetMap[s.id] || [] })),
    );
  }),
);
...
api.get(
  ...
  safeHandler("جلب صلاحيات الشاشة", async (c) => {
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    const rows = await db
      .select({
...
api.post(
  "/screens/:screenId/permissions",
  safeHandler("تعيين صلاحيات الشاشة", async (c) => {
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
...
api.put(
  "/screen-permissions/:id",
  safeHandler("تعديل صلاحية شاشة", async (c) => {
    const id = parseId(c.req.param("id"));
    ...
    const [updated] = await db
      .update(screenPermissions)
      .set(updateData)
      .where(eq(screenPermissions.id, id))
      .returning();
...
api.delete(
  "/screen-permissions/:id",
  safeHandler("حذف صلاحية شاشة", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف الصلاحية غير صالح" }, 400);
...
api.put(
  "/screens/:screenId/permissions/batch",
  safeHandler("تحديث صلاحيات دفعة واحدة", async (c) => {
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    const { permissions } = body;
    if (!permissions || !Array.isArray(permissions))
      return c.json({ error: "قائمة الصلاحيات مطلوبة" }, 400);

    for (const p of permissions) {
      const permission =
        p.canEdit || p.canDelete ? "execute" : p.permission || "view";
      if (p.id) {
        await db
          .update(screenPermissions)
          .set({ permission })
          .where(eq(screenPermissions.id, p.id));
      } else if (p.userId) {
...
api.post(
  "/businesses/:bizId/screens/:screenId/add-to-sidebar",
  bizAuthMiddleware(),
  safeHandler("إضافة شاشة للسايدبار", async (c) => {
    const bizId = getBizId(c);
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    ...
    const [screen] = await db
      ...
      .where(
        and(
          eq(screenTemplates.id, screenId),
          eq(screenTemplates.businessId, bizId),
        ),
      );
    if (!screen)
      return c.json({ error: "الشاشة غير موجودة أو لا تنتمي لهذا العمل" }, 404);
...
// ===================== شاشات المستخدم =====================
api.get(
  "/businesses/:bizId/users/:userId/screens",
  bizAuthMiddleware(),
  safeHandler("جلب شاشات المستخدم", async (c) => {
    const bizId = getBizId(c);
    const userId = parseId(c.req.param("userId"));
    if (!userId) return c.json({ error: "معرّف المستخدم غير صالح" }, 400);
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب إحصائيات العناصر", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب سجل العمليات للعنصر", async (c) => {
    const bizId = getBizId(c);
    const dateFrom = c.req.query("dateFrom");
    const dateTo = c.req.query("dateTo");
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب بيانات مراقبة الحسابات", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب بيانات الرسم البياني", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  "/businesses/:bizId/widget-stats-enhanced",
  bizAuthMiddleware(),
  safeHandler("جلب إحصائيات محسّنة", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  "/businesses/:bizId/widget-log-enhanced",
  bizAuthMiddleware(),
  safeHandler("جلب سجل عمليات محسّن", async (c) => {
    const bizId = getBizId(c);
    const dateFrom = c.req.query("dateFrom");
    const dateTo = c.req.query("dateTo");
    const opTypeId = c.req.query("operationTypeId");
    ...
    if (minAmount)
      conditions = sql`${conditions} AND CAST(je.total_debit AS NUMERIC) >= ${Number.parseFloat(minAmount)}`;
    if (maxAmount)
      conditions = sql`${conditions} AND CAST(je.total_debit AS NUMERIC) <= ${Number.parseFloat(maxAmount)}`;
...
api.get(
  "/businesses/:bizId/widget-chart-enhanced",
  bizAuthMiddleware(),
  safeHandler("جلب بيانات رسم بياني محسّن", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  "/widgets/:widgetId/notes",
  safeHandler("جلب ملاحظات العنصر", async (c) => {
    const widgetId = parseId(c.req.param("widgetId"));
    if (!widgetId) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    const [widget] = await db
      .select({ config: screenWidgets.config })
      .from(screenWidgets)
      .where(eq(screenWidgets.id, widgetId));
...
api.put(
  "/widgets/:widgetId/notes",
  safeHandler("حفظ ملاحظات العنصر", async (c) => {
    const widgetId = parseId(c.req.param("widgetId"));
    if (!widgetId) return c.json({ error: "معرّف العنصر غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    const [widget] = await db
...
api.get(
  "/businesses/:bizId/widget-operation-types",
  bizAuthMiddleware(),
  safeHandler("جلب قوالب العمليات للعنصر", async (c) => {
    const bizId = getBizId(c);
...
// ===================== أسعار الصرف اليومية =====================
api.get(
  "/businesses/:bizId/exchange-rates",
  bizAuthMiddleware(),
  safeHandler("جلب أسعار الصرف", async (c) => {
    const bizId = getBizId(c);
...
api.post(
  "/businesses/:bizId/exchange-rates",
  bizAuthMiddleware(),
  safeHandler("إضافة سعر صرف", async (c) => {
    const bizId = getBizId(c);
    const userId = getUserId(c);
    const body = normalizeBody(await c.req.json());
    if (
      ...
      return c.json(
        {
          error:
            "جميع الحقول مطلوبة: fromCurrencyId, toCurrencyId, rate, effectiveDate",
        },
        400,
      );
    }
...
api.put(
  "/businesses/:bizId/exchange-rates/:id",
  bizAuthMiddleware(),
  safeHandler("تعديل سعر صرف", async (c) => {
    const bizId = getBizId(c);
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
      ...
      .where(and(eq(exchangeRates.id, id), eq(exchangeRates.businessId, bizId)))
      .returning();
...
api.delete(
  "/businesses/:bizId/exchange-rates/:id",
  bizAuthMiddleware(),
  safeHandler("حذف سعر صرف", async (c) => {
    const bizId = getBizId(c);
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    await db
      ...
      .where(
        and(eq(exchangeRates.id, id), eq(exchangeRates.businessId, bizId)),
      );
    return c.json({ success: true });
  }),
);
...
api.get(
  "/businesses/:bizId/exchange-rates/convert",
  bizAuthMiddleware(),
  safeHandler("تحويل عملة", async (c) => {
    const bizId = getBizId(c);
    ...
    if (fromId === toId)
      return c.json({
        rate: 1,
        convertedAmount: Number.parseFloat(amountStr),
    ...
    if (!latestRate) {
      ...
      const [reverseRate] = await db
        ...
        .where(
          and(
            eq(exchangeRates.businessId, bizId),
            eq(exchangeRates.fromCurrencyId, toId),
            eq(exchangeRates.toCurrencyId, fromId),
          ),
        )
        .orderBy(desc(exchangeRates.effectiveDate))
        .limit(1);
      if (!reverseRate)
        return c.json({ error: "لا يوجد سعر صرف بين هاتين العملتين" }, 404);
      const rate = 1 / Number.parseFloat(String(reverseRate.rate));
      return c.json({
        rate,
        convertedAmount: Number.parseFloat(amountStr) * rate,
        fromCurrencyId: fromId,
        toCurrencyId: toId,
      });
    }
...
// ===================== نظام الصلاحيات RBAC =====================
api.get(
  "/businesses/:bizId/roles",
  bizAuthMiddleware(),
  safeHandler("جلب الأدوار", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(roles)
      .where(eq(roles.businessId, bizId))
      .orderBy(roles.name);
  ...
  safeHandler("إنشاء دور", async (c) => {
    ...
    const [created] = await db
      ...
      .values({
        ...
        maxVoucherAmount: body.maxVoucherAmount
          ? String(body.maxVoucherAmount)
          : null,
        maxDailyAmount: body.maxDailyAmount
          ? String(body.maxDailyAmount)
          : null,
      })
      .returning();
    // إضافة الصلاحيات
...
api.put(
  "/businesses/:bizId/roles/:id",
  bizAuthMiddleware(),
  safeHandler("تعديل دور", async (c) => {
    const bizId = getBizId(c);
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
      ...
      .where(and(eq(roles.id, id), eq(roles.businessId, bizId)))
      .returning();
    ...
    if (body.permissions !== undefined) {
      ...
      if (body.permissions && Array.isArray(body.permissions)) {
        for (const p of body.permissions) {
          await db
            .insert(rolePermissions)
            .values({ roleId: id, resource: p.resource, action: p.action });
        }
      }
    }
    return c.json(updated);
...
// تعيين دور لمستخدم
api.post(
  "/businesses/:bizId/user-roles",
  bizAuthMiddleware(),
  safeHandler("تعيين دور لمستخدم", async (c) => {
    const bizId = getBizId(c);
    const assignedBy = getUserId(c);
    const body = normalizeBody(await c.req.json());
    if (!body.userId || !body.roleId)
      return c.json({ error: "userId و roleId مطلوبان" }, 400);
    // حذف الدور القديم إن وجد
    await db
      .delete(userRoles)
      .where(
        and(eq(userRoles.userId, body.userId), eq(userRoles.businessId, bizId)),
      );
    const [created] = await db
      .insert(userRoles)
      .values({
...
api.get(
  "/businesses/:bizId/user-roles",
  bizAuthMiddleware(),
  safeHandler("جلب أدوار المستخدمين", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select({
        id: userRoles.id,
...
api.delete(
  "/businesses/:bizId/user-roles/:userId",
  bizAuthMiddleware(),
  safeHandler("إزالة دور مستخدم", async (c) => {
    const bizId = getBizId(c);
    const userId = parseId(c.req.param("userId"));
    if (!userId) return c.json({ error: "معرّف المستخدم غير صالح" }, 400);
    await db
      .delete(userRoles)
      .where(
        and(eq(userRoles.userId, userId), eq(userRoles.businessId, bizId)),
      );
    return c.json({ success: true });
  }),
);
...
// ===================== المرفقات المحسنة =====================
api.post(
  "/businesses/:bizId/attachments",
  bizAuthMiddleware(),
  safeHandler("رفع مرفق", async (c) => {
    const bizId = getBizId(c);
    const userId = getUserId(c);
    const body = normalizeBody(await c.req.json());
    ...
    const [created] = await db
      ...
      .values({
        ...
        filePath: body.filePath || body.fileUrl || "",
...
api.delete(
  "/businesses/:bizId/attachments/:id",
  bizAuthMiddleware(),
  safeHandler("حذف مرفق", async (c) => {
    const id = parseId(c.req.param("id"));
...
api.post(
  "/businesses/:bizId/vouchers/:id/reverse",
  bizAuthMiddleware(),
  checkPermission("vouchers", "reverse"),
  safeHandler("عكس سند", async (c) => {
    const bizId = getBizId(c);
    const userId = getUserId(c);
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف السند غير صالح" }, 400);
...
api.get(
  "/businesses/:bizId/reports/profit-loss",
  bizAuthMiddleware(),
  safeHandler("تقرير الأرباح والخسائر", async (c) => {
    const bizId = getBizId(c);
    const dateFrom = c.req.query("dateFrom");
    const dateTo = c.req.query("dateTo");
...
// تقرير كشف حساب
api.get(
  "/businesses/:bizId/reports/account-statement/:accountId",
  bizAuthMiddleware(),
  safeHandler("كشف حساب", async (c) => {
    const bizId = getBizId(c);
...
// تقرير ملخص يومي
api.get(
  "/businesses/:bizId/reports/daily-summary",
  bizAuthMiddleware(),
  safeHandler("ملخص يومي", async (c) => {
    const bizId = getBizId(c);
    const dateParam =
...
api.get(
  "/businesses/:bizId/reports/trial-balance",
  bizAuthMiddleware(),
  safeHandler("ميزان المراجعة", async (c) => {
    const bizId = getBizId(c);
    const dateFrom = c.req.query("dateFrom");
    const dateTo = c.req.query("dateTo");
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("تقرير شهري", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  "/reports/aggregated-profit-loss",
  safeHandler("تقرير تجميعي", async (c) => {
    const userId = getUserId(c) || 1;
    const dateFrom = c.req.query("dateFrom");
    const dateTo = c.req.query("dateTo");
    const report = await getAggregatedProfitAndLoss(userId, {
      dateFrom,
      dateTo,
    });
    return c.json(report);
...
api.get(
  ...
  safeHandler("ملخص تجميعي", async (c) => {
    const userId = getUserId(c) || 1;
...
api.get(
  "/businesses/:bizId/vouchers/:voucherId/transitions",
  bizAuthMiddleware(),
  safeHandler("جلب انتقالات سير العمل", async (c) => {
    const bizId = getBizId(c);
...
api.post(
  "/businesses/:bizId/vouchers/:voucherId/transition",
  bizAuthMiddleware(),
  checkPermission("workflow", "execute"),
  safeHandler("تنفيذ انتقال", async (c) => {
    const bizId = getBizId(c);
    const voucherId = parseId(c.req.param("voucherId"));
    if (!voucherId) return c.json({ error: "معرّف السند غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
...
// سجل سير العمل لسند
api.get(
  "/businesses/:bizId/vouchers/:voucherId/workflow-history",
  bizAuthMiddleware(),
  safeHandler("سجل سير العمل", async (c) => {
    const bizId = getBizId(c);
    const voucherId = parseId(c.req.param("voucherId"));
    if (!voucherId) return c.json({ error: "معرّف السند غير صالح" }, 400);
...
api.post(
  "/businesses/:bizId/operation-types/:opTypeId/setup-workflow",
  bizAuthMiddleware(),
  safeHandler("إعداد سير عمل", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  "/businesses/:bizId/operation-types/:opTypeId/transitions",
  bizAuthMiddleware(),
  safeHandler("جلب انتقالات", async (c) => {
    const bizId = getBizId(c);
    const opTypeId = parseId(c.req.param("opTypeId"));
    if (!opTypeId) return c.json({ error: "معرّف نوع العملية غير صالح" }, 400);
...
api.post(
  "/businesses/:bizId/operation-types/:opTypeId/transitions",
  bizAuthMiddleware(),
  safeHandler("إضافة انتقال", async (c) => {
    const bizId = getBizId(c);
    const opTypeId = parseId(c.req.param("opTypeId"));
    if (!opTypeId) return c.json({ error: "معرّف نوع العملية غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
...
// حذف انتقال
api.delete(
  "/businesses/:bizId/transitions/:transitionId",
  bizAuthMiddleware(),
  safeHandler("حذف انتقال", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  "/businesses/:bizId/ui/pages",
  bizAuthMiddleware(),
  safeHandler("جلب الصفحات", async (c) => {
    const bizId = getBizId(c);
...
// جلب صفحة بالمفتاح
api.get(
  "/businesses/:bizId/ui/pages/key/:pageKey",
  bizAuthMiddleware(),
  safeHandler("جلب صفحة", async (c) => {
    const bizId = getBizId(c);
...
// جلب صفحة بالمعرّف
api.get(
  "/businesses/:bizId/ui/pages/:pageId",
  bizAuthMiddleware(),
  safeHandler("جلب صفحة", async (c) => {
    const bizId = getBizId(c);
...
api.post(
  "/businesses/:bizId/ui/pages",
  bizAuthMiddleware(),
  checkPermission("ui_builder", "create"),
  safeHandler("إنشاء صفحة", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    if (!body.pageKey || !body.title)
      return c.json({ error: "المفتاح والعنوان مطلوبان" }, 400);
...
api.put(
  "/businesses/:bizId/ui/pages/:pageId",
  bizAuthMiddleware(),
  safeHandler("تحديث صفحة", async (c) => {
    const bizId = getBizId(c);
    const pageId = parseId(c.req.param("pageId"));
    if (!pageId) return c.json({ error: "معرّف غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
...
// إضافة مكون لصفحة
api.post(
  "/businesses/:bizId/ui/pages/:pageId/components",
  bizAuthMiddleware(),
  safeHandler("إضافة مكون", async (c) => {
    const bizId = getBizId(c);
    const pageId = parseId(c.req.param("pageId"));
    if (!pageId) return c.json({ error: "معرّف غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
...
// تحديث مكون
api.put(
  "/businesses/:bizId/ui/components/:componentId",
  bizAuthMiddleware(),
  safeHandler("تحديث مكون", async (c) => {
    const bizId = getBizId(c);
...
// جلب مصادر البيانات
api.get(
  "/businesses/:bizId/ui/data-sources",
  bizAuthMiddleware(),
  safeHandler("جلب مصادر بيانات", async (c) => {
    const bizId = getBizId(c);
...
api.post(
  "/businesses/:bizId/ui/data-sources",
  bizAuthMiddleware(),
  safeHandler("إنشاء مصدر بيانات", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    if (!body.name || !body.sourceType)
      return c.json({ error: "الاسم والنوع مطلوبان" }, 400);
...
// تحديث مصدر بيانات
api.put(
  "/businesses/:bizId/ui/data-sources/:dsId",
  bizAuthMiddleware(),
  safeHandler("تحديث مصدر بيانات", async (c) => {
    const bizId = getBizId(c);
...
api.delete(
  "/businesses/:bizId/ui/data-sources/:dsId",
  bizAuthMiddleware(),
  safeHandler("حذف مصدر بيانات", async (c) => {
    const bizId = getBizId(c);
    const dsId = parseId(c.req.param("dsId"));
    if (!dsId) return c.json({ error: "معرّف غير صالح" }, 400);
...
// تنفيذ مصدر بيانات
api.post(
  "/businesses/:bizId/ui/data-sources/:dsId/execute",
  bizAuthMiddleware(),
  safeHandler("تنفيذ مصدر بيانات", async (c) => {
    const bizId = getBizId(c);
    const dsId = parseId(c.req.param("dsId"));
    if (!dsId) return c.json({ error: "معرّف غير صالح" }, 400);
    const body = normalizeBody(await c.req.json());
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("جلب إعداد الشاشة المخصصة", async (c) => {
    const bizId = getBizId(c);
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
    ...
    if (
      layoutConfig.tabs &&
      Array.isArray(layoutConfig.tabs) &&
      layoutConfig.tabs.length > 0
    ) {
      return c.json({
        screenId,
        tabs: layoutConfig.tabs,
        notes: layoutConfig.notes || "",
      });
    }
    ...
    if (oldConfig) {
      const tabs: any[] = [];
      if (
        oldConfig.tab1OperationTypeIds &&
        (oldConfig.tab1OperationTypeIds as number[]).length > 0
      ) {
        tabs.push({
          id: "tab_1",
          label: oldConfig.tab1Label,
          icon: oldConfig.tab1Icon,
          color: oldConfig.tab1Color,
          type: "operations",
          sortOrder: 1,
          config: { operationTypeIds: oldConfig.tab1OperationTypeIds },
        });
      }
      if (
        oldConfig.tab2OperationTypeIds &&
        (oldConfig.tab2OperationTypeIds as number[]).length > 0
      ) {
        tabs.push({
          id: "tab_2",
          label: oldConfig.tab2Label,
          icon: oldConfig.tab2Icon,
          color: oldConfig.tab2Color,
          type: "operations",
          sortOrder: 2,
          config: { operationTypeIds: oldConfig.tab2OperationTypeIds },
        });
      }
      tabs.push({
        id: "tab_history",
        label: oldConfig.historyLabel,
        icon: oldConfig.historyIcon,
        color: oldConfig.historyColor,
        type: "log",
        sortOrder: tabs.length + 1,
        config: {},
      });
      if (
        oldConfig.accountIds &&
        (oldConfig.accountIds as number[]).length > 0
      ) {
        tabs.push({
          id: "tab_accounts",
          label: oldConfig.accountsSectionLabel,
          icon: oldConfig.accountsIcon,
          color: oldConfig.accountsColor,
          type: "accounts",
          sortOrder: tabs.length + 1,
          config: { accountIds: oldConfig.accountIds },
        });
      }
...
api.put(
  "/businesses/:bizId/screens/:screenId/collection-style-config",
  bizAuthMiddleware(),
  safeHandler("حفظ إعداد الشاشة المخصصة", async (c) => {
    const bizId = getBizId(c);
    const screenId = parseId(c.req.param("screenId"));
    if (!screenId) return c.json({ error: "معرّف الشاشة غير صالح" }, 400);
...
// ===================== تصنيفات المخازن =====================
api.get(
  "/businesses/:bizId/warehouse-types",
  bizAuthMiddleware(),
  safeHandler("جلب تصنيفات المخازن", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(warehouseTypes)
      .where(eq(warehouseTypes.businessId, bizId))
      .orderBy(warehouseTypes.sortOrder);
    return c.json(rows);
  }),
);
...
api.post(
  "/businesses/:bizId/warehouse-types",
  bizAuthMiddleware(),
  safeHandler("إضافة تصنيف مخزن", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const validation = validateBody(typeSchema, body);
    if (!validation.success) return c.json({ error: validation.error }, 400);
    ...
    const existing = await db
      ...
      .where(
        and(
          eq(warehouseTypes.businessId, bizId),
          eq(warehouseTypes.subTypeKey, subTypeKey),
        ),
      )
      .limit(1);
    if (existing.length > 0)
...
api.put(
  "/warehouse-types/:id",
  safeHandler("تعديل تصنيف مخزن", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف التصنيف غير صالح" }, 400);
    const [rec] = await db
      .select()
      .from(warehouseTypes)
      .where(eq(warehouseTypes.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
    if (err) return err;
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
      .update(warehouseTypes)
      .set({ ...body, updatedAt: new Date() })
      .where(eq(warehouseTypes.id, id))
      .returning();
...
api.delete(
  "/warehouse-types/:id",
  safeHandler("حذف تصنيف مخزن", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف التصنيف غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.get(
  "/businesses/:bizId/journal-entry-categories",
  bizAuthMiddleware(),
  safeHandler("جلب تصنيفات قيود اليومية", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select()
      .from(journalEntryCategories)
      .where(eq(journalEntryCategories.businessId, bizId))
      .orderBy(journalEntryCategories.sortOrder);
    return c.json(rows);
  }),
);
...
api.post(
  "/businesses/:bizId/journal-entry-categories",
  bizAuthMiddleware(),
  safeHandler("إضافة تصنيف قيد يومية", async (c) => {
    const bizId = getBizId(c);
    const body = normalizeBody(await c.req.json());
    const validation = validateBody(typeSchema, body);
    if (!validation.success) return c.json({ error: validation.error }, 400);
    ...
    const existing = await db
      .select({ id: journalEntryCategories.id })
      .from(journalEntryCategories)
      .where(
        and(
          eq(journalEntryCategories.businessId, bizId),
    ...
    let sortOrder =
      typeof (rest as any).sortOrder === "number" &&
      Number.isInteger((rest as any).sortOrder) &&
      (rest as any).sortOrder > 0
    ...
    `);
      const next = Number(
        getFirstRow<{ next: string | number }>(r as any)?.next ?? 1,
      );
      sortOrder = Number.isFinite(next) && next > 0 ? next : 1;
    }

    const [created] = await db
      .insert(journalEntryCategories)
      .values({
        ...(rest as any),
        sortOrder,
        categoryKey,
...
api.put(
  "/journal-entry-categories/:id",
  safeHandler("تعديل تصنيف قيد يومية", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف التصنيف غير صالح" }, 400);
    const [rec] = await db
      .select()
      .from(journalEntryCategories)
      .where(eq(journalEntryCategories.id, id));
    const err = await requireResourceOwnership(c, rec ?? null);
    if (err) return err;
    const body = normalizeBody(await c.req.json());
    const [updated] = await db
      .update(journalEntryCategories)
      .set({ ...body, updatedAt: new Date() })
      .where(eq(journalEntryCategories.id, id))
      .returning();
    if (!updated) return c.json({ error: "التصنيف غير موجود" }, 404);
...
api.delete(
  "/journal-entry-categories/:id",
  safeHandler("حذف تصنيف قيد يومية", async (c) => {
    const id = parseId(c.req.param("id"));
    if (!id) return c.json({ error: "معرّف التصنيف غير صالح" }, 400);
    const [rec] = await db
      .select()
...
api.get(
  "/businesses/:bizId/warehouses/:warehouseId/operations",
  bizAuthMiddleware(),
  safeHandler("جلب عمليات المخزن", async (c) => {
    const bizId = getBizId(c);
    ...
    const rows = await db
      ...
      .leftJoin(
        operationTypes,
        eq(warehouseOperations.operationTypeId, operationTypes.id),
      )
      .where(
        and(
      ...
      .orderBy(
        desc(warehouseOperations.operationDate),
        desc(warehouseOperations.id),
      );
...
// إنشاء عملية مخزنية
api.post(
  "/businesses/:bizId/warehouse-operations",
  bizAuthMiddleware(),
  checkPermission("inventory", "create"),
  safeHandler("إنشاء عملية مخزنية", async (c) => {
    const bizId = getBizId(c);
    ...
    if (body.operationTypeId) {
      const otRows = await db.execute(
        sql`SELECT * FROM operation_types WHERE id = ${body.operationTypeId} AND business_id = ${bizId}`,
      );
      opTemplate = getFirstRow<Record<string, unknown>>(otRows) ?? null;
      if (opTemplate) {
        // تطبيق خصائص القالب إذا لم يتم تحديدها يدوياً
        if (!body.operationType && opTemplate.voucher_type)
          body.operationType = opTemplate.voucher_type;
        if (!body.sourceWarehouseId && opTemplate.source_warehouse_id)
          body.sourceWarehouseId = opTemplate.source_warehouse_id;
        if (!body.description && opTemplate.name)
          body.description = opTemplate.name;
      }
    }

    // التحقق من الحقول الإلزامية
    if (!body.operationTypeId)
      return c.json({ error: "معرّف نوع العملية (القالب) مطلوب" }, 400);
    if (!body.operationType)
      return c.json({ error: "نوع العملية المخزنية مطلوب" }, 400);
    ...
    if (body.sourceWarehouseId) {
      const [srcWh] = await db
        ...
        .where(
          and(
            eq(warehouses.id, body.sourceWarehouseId),
            eq(warehouses.businessId, bizId),
          ),
        );
      if (!srcWh)
        return c.json(
          { error: "المخزن المصدر غير موجود أو لا ينتمي لهذا العمل" },
          400,
        );
    }
    if (body.destinationWarehouseId) {
      const [destWh] = await db
        .select()
        .from(warehouses)
        .where(
          and(
            eq(warehouses.id, body.destinationWarehouseId),
            eq(warehouses.businessId, bizId),
          ),
        );
      if (!destWh)
        return c.json(
          { error: "المخزن الوجهة غير موجود أو لا ينتمي لهذا العمل" },
          400,
        );
    }
    ...
    if (!validTypes.includes(body.operationType)) {
      return c.json(
        {
          error: `نوع العملية غير صالح. الأنواع المتاحة: ${validTypes.join(", ")}`,
        },
        400,
      );
    }
    ...
    if (body.operationType === "receive_transfer" && !body.relatedOperationId) {
      return c.json(
        {
          error:
            "استلام التحويل يتطلب تحديد عملية التحويل المرتبطة (relatedOperationId)",
        },
        400,
      );
    }
    ...
    const [mainWh] = await db
      ...
      .where(
        and(
          eq(warehouses.id, mainWarehouseId),
          eq(warehouses.businessId, bizId),
        ),
      )
      .limit(1);
    if (!mainWh)
      return c.json(
        { error: "المخزن المحدد غير موجود أو لا ينتمي لهذا العمل" },
        400,
      );
    ...
    if (mainWh.subTypeId) {
      const [whCategory] = await db
        .select({ sequenceNumber: warehouseTypes.sequenceNumber })
        .from(warehouseTypes)
        .where(eq(warehouseTypes.id, mainWh.subTypeId));
    ...
    if (body.operationTypeId) {
      ...
      const tRaw = await getNextSequence(
        bizId,
        tCounter,
    ...
    for (const item of body.items) {
      ...
      const existingItems = await db
        ...
        .where(
          and(
            ...
            itemCode
              ? eq(inventoryItems.code, itemCode)
      ...
      if (existingItems.length > 0) {
        ...
        const autoCode =
          itemCode ||
          `${itemName.substring(0, 3).toUpperCase()}-${String(Math.floor(Math.random() * 9999)).padStart(4, "0")}`;
      ...
      try {
        const warehouseId =
          body.sourceWarehouseId || body.destinationWarehouseId;
        if (
          ["supply_invoice", "supply_order", "receive_transfer"].includes(
            body.operationType,
          )
        ) {
        ...
        console.error(
          `تحذير: فشل تحديث المخزون للصنف ${itemName}:`,
          toErrorMessage(invErr),
        );
      }
    }
    ...
    const sourceAccountId = opTemplate
      ? (opTemplate.source_account_id as number | undefined)
      : undefined;
    if (opTemplate?.auto_journal === true && sourceAccountId) {
      try {
        const txResult = await postTransaction(bizId, userId, {
          voucherType:
            body.operationType === "dispatch" ? "payment" : "receipt",
          amount: totalCost,
          currencyId: body.currencyId || 1,
        ...
        console.error(
          "تحذير: فشل إنشاء القيد المحاسبي:",
          toErrorMessage(txErr),
        );
      }
    }
...
api.get(
  ...
  safeHandler("جلب تفاصيل عملية مخزنية", async (c) => {
    const id = parseId(c.req.param("id"));
    ...
    const [operation] = await db
      .select()
      .from(warehouseOperations)
      .where(eq(warehouseOperations.id, id));
    if (!operation)
      return c.json({ error: "العملية المخزنية غير موجودة" }, 404);
    const opErr = await requireResourceOwnership(c, operation);
    if (opErr) return opErr;

    const items = await db
      .select()
      .from(warehouseOperationItems)
    ...
    if (operation.operationTypeId) {
      const [ot] = await db
        .select()
        .from(operationTypes)
        .where(eq(operationTypes.id, operation.operationTypeId));
    ...
    if (operation.sourceWarehouseId) {
      const [w] = await db
        .select()
        .from(warehouses)
        .where(eq(warehouses.id, operation.sourceWarehouseId));
    ...
    if (operation.destinationWarehouseId) {
      const [w] = await db
        .select()
        .from(warehouses)
        .where(eq(warehouses.id, operation.destinationWarehouseId));
...
api.get(
  "/businesses/:bizId/warehouses/:warehouseId/inventory",
  bizAuthMiddleware(),
  safeHandler("جلب مخزون المخزن", async (c) => {
    const bizId = getBizId(c);
    const warehouseId = parseId(c.req.param("warehouseId"));
    if (!warehouseId) return c.json({ error: "معرّف المخزن غير صالح" }, 400);
...
api.get(
  "/businesses/:bizId/warehouse-operations",
  bizAuthMiddleware(),
  safeHandler("جلب كل العمليات المخزنية", async (c) => {
    const bizId = getBizId(c);
    ...
    if (opType)
      conditions.push(eq(warehouseOperations.operationType, opType as any));
    if (warehouseId) {
      const whId = Number.parseInt(warehouseId);
      conditions.push(
        sql`(${warehouseOperations.sourceWarehouseId} = ${whId} OR ${warehouseOperations.destinationWarehouseId} = ${whId})`,
      );
    }
...
api.get(
  "/businesses/:bizId/inventory-summary",
  bizAuthMiddleware(),
  safeHandler("ملخص مخزون عدة مخازن", async (c) => {
    const bizId = getBizId(c);
    const warehouseIdsParam = c.req.query("warehouseIds");
    if (!warehouseIdsParam) return c.json({ error: "يجب تحديد المخازن" }, 400);
    const warehouseIds = warehouseIdsParam
      .split(",")
      .map(Number)
      .filter((n: number) => !Number.isNaN(n) && n > 0);
    if (warehouseIds.length === 0)
      return c.json({ error: "معرّفات المخازن غير صالحة" }, 400);
    ...
    const totalCost = allInventory.reduce(
...
api.get(
  "/businesses/:bizId/warehouse-operations-summary",
  bizAuthMiddleware(),
  safeHandler("ملخص العمليات المخزنية", async (c) => {
    const bizId = getBizId(c);
    ...
    if (from && !dateRegex.test(from))
      return c.json(
        { error: "تنسيق تاريخ البداية غير صالح (YYYY-MM-DD)" },
        400,
      );
    if (to && !dateRegex.test(to))
      return c.json(
        { error: "تنسيق تاريخ النهاية غير صالح (YYYY-MM-DD)" },
        400,
      );
...
api.get(
  "/businesses/:bizId/stock-levels",
  bizAuthMiddleware(),
  checkPermission("inventory", "read"),
  safeHandler("أرصدة المخزون", async (c) => {
    const bizId = getBizId(c);
    const warehouseId = c.req.query("warehouseId")
      ? Number.parseInt(c.req.query("warehouseId")!)
      : undefined;
...
api.get(
  "/businesses/:bizId/stock-alerts",
  bizAuthMiddleware(),
  checkPermission("inventory", "read"),
  safeHandler("تنبيهات المخزون", async (c) => {
    const bizId = getBizId(c);
...
// تقييم المخزون
api.get(
  "/businesses/:bizId/stock-valuation",
  bizAuthMiddleware(),
  checkPermission("inventory", "read"),
  safeHandler("تقييم المخزون", async (c) => {
    const bizId = getBizId(c);
    const warehouseId = c.req.query("warehouseId")
      ? Number.parseInt(c.req.query("warehouseId")!)
      : undefined;
...
// سجل حركات صنف
api.get(
  "/businesses/:bizId/items/:itemId/movements",
  bizAuthMiddleware(),
  checkPermission("inventory", "read"),
  safeHandler("حركات صنف", async (c) => {
    const bizId = getBizId(c);
...
api.post(
  "/businesses/:bizId/stock-movements",
  bizAuthMiddleware(),
  checkPermission("inventory", "create"),
  safeHandler("تسجيل حركة مخزون", async (c) => {
    const bizId = getBizId(c);
    const userId = getUserId(c);
    const body = normalizeBody(await c.req.json());
    ...
    if (
      ...
      return c.json(
        {
          error:
            "البيانات المطلوبة: itemId, warehouseId, movementType, quantity, movementDate",
        },
        400,
      );
    }
