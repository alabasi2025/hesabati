...
import {
  getUserMaxAmounts,
  checkResourceLimit,
} from "../../engines/permissions.engine.ts";
...
api.get(
  ...
  bizAuthMiddleware(),
  safeHandler("ط¬ظ„ط¨ ط§ظ„ط£ط¯ظˆط§ط±", async (c) => {
...
api.post(
  "/businesses/:bizId/roles",
  bizAuthMiddleware(),
  safeHandler("ط¥ظ†ط´ط§ط، ط¯ظˆط±", async (c) => {
    const bizId = getBizId(c);
...
api.put(
  ...
  bizAuthMiddleware(),
  safeHandler("طھط¹ط¯ظٹظ„ ط¯ظˆط±", async (c) => {
    const bizId = getBizId(c);
...
api.delete(
  "/businesses/:bizId/roles/:id",
  bizAuthMiddleware(),
  safeHandler("ط­ط°ظپ ط¯ظˆط±", async (c) => {
    const bizId = getBizId(c);
    const id = parseId(c.req.param("id"));
    ...
    if (role.isSystem)
      return c.json({ error: "ظ„ط§ ظٹظ…ظƒظ† ط­ط°ظپ ط¯ظˆط± ط§ظ„ظ†ط¸ط§ظ…" }, 400);
    await db.delete(rolePermissions).where(eq(rolePermissions.roleId, id));
...
api.post(
  ...
  bizAuthMiddleware(),
  safeHandler("طھط¹ظٹظٹظ† ط¯ظˆط± ظ„ظ…ط³طھط®ط¯ظ…", async (c) => {
    const bizId = getBizId(c);
...
api.get(
  "/businesses/:bizId/user-roles",
  bizAuthMiddleware(),
  safeHandler("ط¬ظ„ط¨ ط£ط¯ظˆط§ط± ط§ظ„ظ…ط³طھط®ط¯ظ…ظٹظ†", async (c) => {
    const bizId = getBizId(c);
    const rows = await db
      .select({
        id: userRoles.id,
...
api.delete(
  ...
  bizAuthMiddleware(),
  safeHandler("ط¥ط²ط§ظ„ط© ط¯ظˆط± ظ…ط³طھط®ط¯ظ…", async (c) => {
    const bizId = getBizId(c);
    const userId = parseId(c.req.param("userId"));
    if (!userId)
      return c.json(
        { error: "ظ…ط¹ط±ظ‘ظپ ط§ظ„ظ…ط³طھط®ط¯ظ… ط؛ظٹط± طµط§ظ„ط­" },
        400,
      );
    await db
      .delete(userRoles)
