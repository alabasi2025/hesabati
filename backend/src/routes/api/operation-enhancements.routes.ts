...
operationEnhRouter.post(
  "/businesses/:bizId/operation-types/:id/clone",
  bizAuthMiddleware(),
  safeHandler("ظ†ط³ط® ظ†ظˆط¹ ط¹ظ…ظ„ظٹط©", async (c) => {
    const bizId = getBizId(c);
    const id = parseId(c.req.param("id"));
    if (!id)
      return c.json(
        { error: "ظ…ط¹ط±ظ‘ظپ ظ†ظˆط¹ ط§ظ„ط¹ظ…ظ„ظٹط© ط؛ظٹط± طµط§ظ„ط­" },
        400,
      );
    ...
    const [original] = await db
      ...
      .from(operationTypes)
      .where(
        and(eq(operationTypes.id, id), eq(operationTypes.businessId, bizId)),
      );
    if (!original)
      return c.json({ error: "ظ†ظˆط¹ ط§ظ„ط¹ظ…ظ„ظٹط© ط؛ظٹط± ظ…ظˆط¬ظˆط¯" }, 404);

    const body = await getBody(c);
    const newName = body.name || `${original.name} (ظ†ط³ط®ط©)`;
    const normalizedCategoryId =
      Number.isInteger(original.categoryId) && Number(original.categoryId) > 0
    ...
    if (normalizedCategoryId > 0) {
      const [cat] = await db
        ...
        .where(
          and(
            eq(operationCategories.id, normalizedCategoryId),
...
operationEnhRouter.post(
  "/businesses/:bizId/operation-types/:id/toggle",
  bizAuthMiddleware(),
  safeHandler("طھظپط¹ظٹظ„/طھط¹ط·ظٹظ„ ظ†ظˆط¹ ط¹ظ…ظ„ظٹط©", async (c) => {
    const bizId = getBizId(c);
    const id = parseId(c.req.param("id"));
    if (!id)
      return c.json(
      ...
      if (vt === "receipt" || vt === "payment") {
        ...
        if (!pm)
          return c.json(
            {
              error:
                "ظ„ط§ ظٹظ…ظƒظ† طھظپط¹ظٹظ„ ظ‡ط°ط§ ط§ظ„ظ‚ط§ظ„ط¨ ظ‚ط¨ظ„ طھط­ط¯ظٹط¯ ظˆط³ظٹظ„ط© ط§ظ„ط¯ظپط¹ ظˆط§ظ„ط®ط²ظٹظ†ط©",
            },
            400,
          );
        if (pm === "cash") {
          if (!existing.sourceFundId)
            return c.json(
              {
                error:
                  "ظ„ط§ ظٹظ…ظƒظ† طھظپط¹ظٹظ„ ظ‡ط°ط§ ط§ظ„ظ‚ط§ظ„ط¨ ظ‚ط¨ظ„ طھط­ط¯ظٹط¯ ط§ظ„ط®ط²ظٹظ†ط© (ط§ظ„طµظ†ط¯ظˆظ‚)",
              },
              400,
            );
        } else {
          if (!existing.sourceAccountId)
            return c.json(
              {
                error:
                  "ظ„ط§ ظٹظ…ظƒظ† طھظپط¹ظٹظ„ ظ‡ط°ط§ ط§ظ„ظ‚ط§ظ„ط¨ ظ‚ط¨ظ„ طھط­ط¯ظٹط¯ ط§ظ„ط®ط²ظٹظ†ط© (ط­ط³ط§ط¨ ط¨ظ†ظƒ/طµط±ط§ظپ/ظ…ط­ظپط¸ط©)",
              },
              400,
            );
        }
        ...
        if (!cntRow?.cnt)
          return c.json(
            {
              error:
                "ظ„ط§ ظٹظ…ظƒظ† طھظپط¹ظٹظ„ ظ‡ط°ط§ ط§ظ„ظ‚ط§ظ„ط¨ ظ‚ط¨ظ„ ط¥ط¶ط§ظپط© ط­ط³ط§ط¨ ظˆط§ط­ط¯ ط¹ظ„ظ‰ ط§ظ„ط£ظ‚ظ„ ظپظٹ ط§ظ„ط­ط³ط§ط¨ط§طھ ط§ظ„ظ…ط±طھط¨ط·ط©",
            },
            400,
          );
      }
...
operationEnhRouter.get(
  "/businesses/:bizId/operation-types-stats",
  bizAuthMiddleware(),
  safeHandler("ط¥ط­طµط§ط¦ظٹط§طھ ط£ظ†ظˆط§ط¹ ط§ظ„ط¹ظ…ظ„ظٹط§طھ", async (c) => {
    const bizId = getBizId(c);
...
operationEnhRouter.get(
  "/businesses/:bizId/operation-types/check-name",
  bizAuthMiddleware(),
  safeHandler("ط§ظ„طھط­ظ‚ظ‚ ظ…ظ† طھظƒط±ط§ط± ط§ط³ظ…", async (c) => {
    const bizId = getBizId(c);
    ...
    if (excludeId)
      conditions = sql`${conditions} AND id != ${parseInt(excludeId)}`;

    const result = await db.execute(
      sql`SELECT COUNT(*) as cnt FROM operation_types WHERE ${conditions}`,
    );
