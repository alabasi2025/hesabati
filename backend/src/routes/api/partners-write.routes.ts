...
partnersWriteRoutes.post(
  ...
  safeHandler("إضافة شريك", async (c) => {
    ...
    if (!accountId)
      return c.json(
        { error: "يجب اختيار حساب مرتبط بالشريك من دليل الحسابات" },
        400,
      );
    ...
    const existingPartners = await db
      ...
      .where(
        and(
          ...
          eq(businessPartners.accountId, accountId),
        ),
      );
    ...
    const partnerSharePercentage =
      ...
      typeof body.sharePercentage === "string"
...
partnersWriteRoutes.put(
  ...
  safeHandler("تعديل شريك", async (c) => {
    ...
    if (!existing)
      return c.json({ error: "شريك غير موجود أو لا ينتمي لهذا العمل" }, 404);
    const body = (await getBody(c)) as Record<string, unknown>;
