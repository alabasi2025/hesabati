...
interface PeriodStatsRow {
  ...
  operations_count: number | string;
}
...
screenEnhRouter.get(
  ...
  bizAuthMiddleware(),
  safeHandler("ط³ط¬ظ„ ط§ظ„ط¹ظ…ظ„ظٹط§طھ ط§ظ„ظ…ط­ط³ظ†", async (c) => {
    ...
    if (opTypeId)
      conditions = sql`${conditions} AND je.operation_type_id = ${parseInt(opTypeId)}`;
    if (search)
      conditions = sql`${conditions} AND (je.description ILIKE ${"%" + search + "%"} OR je.entry_number ILIKE ${"%" + search + "%"} OR je.reference ILIKE ${"%" + search + "%"})`;
    if (minAmount)
      conditions = sql`${conditions} AND CAST(je.total_debit AS NUMERIC) >= ${parseFloat(minAmount)}`;
    if (maxAmount)
      conditions = sql`${conditions} AND CAST(je.total_debit AS NUMERIC) <= ${parseFloat(maxAmount)}`;
  ...
  `);
    ...
    const countResult = await db.execute(
      sql`SELECT COUNT(*) as total FROM journal_entries je WHERE ${conditions}`,
    );
...
screenEnhRouter.get(
  ...
  bizAuthMiddleware(),
  safeHandler("ط¥ط­طµط§ط¦ظٹط§طھ ظ…طھظ‚ط¯ظ…ط©", async (c) => {
    const bizId = getBizId(c);
    ...
    if (dateFrom && dateTo) {
      ...
      prevFrom = new Date(new Date(dateFrom).getTime() - diff)
        .toISOString()
        .split("T")[0];
      prevTo = new Date(new Date(dateFrom).getTime() - 1)
        .toISOString()
        .split("T")[0];
    } else {
      switch (period) {
        ...
        case "week":
          ...
          prevTo = new Date(weekStart.getTime() - 86400000)
            .toISOString()
            .split("T")[0];
          break;
        case "year":
  ...
  `);
    ...
    const calcChange = (cur: number, prv: number) =>
      prv === 0 ? (cur > 0 ? 100 : 0) : ((cur - prv) / prv) * 100;
    ...
    return c.json({
      ...
      changes: {
        receiptsChange: calcChange(
          Number(current.receipts),
          Number(prev.receipts),
        ),
        paymentsChange: calcChange(
          Number(current.payments),
          Number(prev.payments),
        ),
        netChange: calcChange(
          Number(current.receipts) - Number(current.payments),
          Number(prev.receipts) - Number(prev.payments),
        ),
        operationsChange: calcChange(
          Number(current.operations_count),
          Number(prev.operations_count),
        ),
      },
    });
  }),
...
screenEnhRouter.get(
  ...
  bizAuthMiddleware(),
  safeHandler("ط±ط³ظ… ط¨ظٹط§ظ†ظٹ ظ…طھظ‚ط¯ظ…", async (c) => {
    const bizId = getBizId(c);
...
screenEnhRouter.post(
  "/businesses/:bizId/vouchers-draft",
  bizAuthMiddleware(),
  safeHandler("ط¥ظ†ط´ط§ط، ط³ظ†ط¯ ظƒظ…ط³ظˆط¯ط©", async (c) => {
    const bizId = getBizId(c);
    ...
    if (isNaN(amount) || amount <= 0)
      return c.json(
        { error: "ط§ظ„ظ…ط¨ظ„ط؛ ظٹط¬ط¨ ط£ظ† ظٹظƒظˆظ† ط±ظ‚ظ…ط§ظ‹ ظ…ظˆط¬ط¨ط§ظ‹" },
        400,
      );
    ...
    if (!treasury) {
      return c.json(
        {
          error:
            "ظ„ط§ ظٹظ…ظƒظ† طھظˆظ„ظٹط¯ ط±ظ‚ظ… ط§ظ„ط³ظ†ط¯: طھط£ظƒط¯ ظ…ظ† ط§ط®طھظٹط§ط± ط®ط²ظٹظ†ط© ظ…ط±ظ‚ظ…ط© ط¨ط´ظƒظ„ طµط­ظٹط­",
        },
        400,
      );
    }
