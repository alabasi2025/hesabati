...
import { generateWarehouseOpFullSequence } from "../../middleware/sequencing.ts";
...
warehouseOpsWriteRoutes.get(
  "/businesses/:bizId/warehouses/:warehouseId/operations",
  bizAuthMiddleware(),
  safeHandler("ط¬ظ„ط¨ ط¹ظ…ظ„ظٹط§طھ ط§ظ„ظ…ط®ط²ظ†", async (c) => {
    const bizId = getBizId(c);
    const warehouseId = parseId(c.req.param("warehouseId"));
    if (!warehouseId)
      return c.json({ error: "ظ…ط¹ط±ظ‘ظپ ط§ظ„ظ…ط®ط²ظ† ط؛ظٹط± طµط§ظ„ط­" }, 400);
...
warehouseOpsWriteRoutes.post(
  "/businesses/:bizId/warehouse-operations",
  bizAuthMiddleware(),
  checkPermission("inventory", "create"),
  safeHandler("ط¥ظ†ط´ط§ط، ط¹ظ…ظ„ظٹط© ظ…ط®ط²ظ†ظٹط©", async (c) => {
    const bizId = getBizId(c);
    ...
    if (!body.operationTypeId)
      return c.json(
        { error: "ظ…ط¹ط±ظ‘ظپ ظ†ظˆط¹ ط§ظ„ط¹ظ…ظ„ظٹط© (ط§ظ„ظ‚ط§ظ„ط¨) ظ…ط·ظ„ظˆط¨" },
        400,
      );
    if (!body.operationType)
      return c.json(
        { error: "ظ†ظˆط¹ ط§ظ„ط¹ظ…ظ„ظٹط© ط§ظ„ظ…ط®ط²ظ†ظٹط© ظ…ط·ظ„ظˆط¨" },
        400,
      );

    const userPermissions = c.get("userPermissions") as
      | { isAdmin?: boolean; constraints?: unknown }
      | undefined;
    const constraintsCheck = validateConstraints(userPermissions ?? {}, {
      operationTypeId: body.operationTypeId,
    });
    if (!constraintsCheck.valid)
      return c.json({ error: constraintsCheck.error }, 403);

    if (!body.sourceWarehouseId && !body.destinationWarehouseId) {
      return c.json(
        { error: "ظٹط¬ط¨ طھط­ط¯ظٹط¯ ظ…ط®ط²ظ† ظ…طµط¯ط± ط£ظˆ ظ…ط®ط²ظ† ظˆط¬ظ‡ط©" },
        400,
      );
    }
    if (!body.items || !Array.isArray(body.items) || body.items.length === 0) {
      return c.json(
        { error: "ظٹط¬ط¨ ط¥ط¶ط§ظپط© طµظ†ظپ ظˆط§ط­ط¯ ط¹ظ„ظ‰ ط§ظ„ط£ظ‚ظ„" },
        400,
      );
    }
    ...
    if (body.sourceWarehouseId) {
      ...
      if (!srcWh)
        return c.json(
          {
            error:
              "ط§ظ„ظ…ط®ط²ظ† ط§ظ„ظ…طµط¯ط± ط؛ظٹط± ظ…ظˆط¬ظˆط¯ ط£ظˆ ظ„ط§ ظٹظ†طھظ…ظٹ ظ„ظ‡ط°ط§ ط§ظ„ط¹ظ…ظ„",
          },
          400,
        );
    }
    if (body.destinationWarehouseId) {
      ...
      if (!destWh)
        return c.json(
          {
            error:
              "ط§ظ„ظ…ط®ط²ظ† ط§ظ„ظˆط¬ظ‡ط© ط؛ظٹط± ظ…ظˆط¬ظˆط¯ ط£ظˆ ظ„ط§ ظٹظ†طھظ…ظٹ ظ„ظ‡ط°ط§ ط§ظ„ط¹ظ…ظ„",
          },
          400,
        );
    }
    ...
    if (!validTypes.includes(body.operationType)) {
      return c.json(
        {
          error: `ظ†ظˆط¹ ط§ظ„ط¹ظ…ظ„ظٹط© ط؛ظٹط± طµط§ظ„ط­. ط§ظ„ط£ظ†ظˆط§ط¹ ط§ظ„ظ…طھط§ط­ط©: ${validTypes.join(", ")}`,
        },
        400,
      );
    }
    const opTypeToCounterType: Record<
      string,
      | "receive_purchase"
      | "direct_supply"
      | "dispatch"
    ...
    if (body.operationType === "receive_transfer" && !body.relatedOperationId) {
      return c.json(
        {
          error:
            "ط§ط³طھظ„ط§ظ… ط§ظ„طھط­ظˆظٹظ„ ظٹطھط·ظ„ط¨ طھط­ط¯ظٹط¯ ط¹ظ…ظ„ظٹط© ط§ظ„طھط­ظˆظٹظ„ ط§ظ„ظ…ط±طھط¨ط·ط© (relatedOperationId)",
        },
        400,
      );
    }
    ...
    const [mainWh] = await db
      .select()
      .from(warehouses)
      .where(eq(warehouses.id, mainWarehouseId));
    ...
    const tmplSeq = 0; // Template sequence - reserved for future use
    ...
    const [created] = await db
      ...
      .values({
        ...
        operationDate:
          body.operationDate || new Date().toISOString().split("T")[0],
    ...
    for (const item of body.items) {
      ...
      if (existingItems.length > 0) {
        ...
        if (
          ...
          existingItems[0].itemTypeId !== item.itemTypeId
        ) {
          await db
        ...
        const [newItem] = await db
          ...
          .values({
            ...
            code: autoCode,
            itemTypeId:
              typeof item.itemTypeId === "number" ? item.itemTypeId : null,
            unit: item.unit || null,
          })
          .returning();
      ...
      try {
        ...
        console.error(
          `طھط­ط°ظٹط±: ظپط´ظ„ طھط­ط¯ظٹط« ط§ظ„ظ…ط®ط²ظˆظ† ظ„ظ„طµظ†ظپ ${itemName}:`,
    ...
    if (opTemplate?.auto_journal === true && sourceAccountId) {
      try {
        ...
        console.error(
          "طھط­ط°ظٹط±: ظپط´ظ„ ط¥ظ†ط´ط§ط، ط§ظ„ظ‚ظٹط¯ ط§ظ„ظ…ط­ط§ط³ط¨ظٹ:",
