/**
 * Attachments Routes â€” ظ…ط³ط§ط±ط§طھ ط§ظ„ظ…ط±ظپظ‚ط§طھ ط§ظ„ظ…ط­ط³ظ†ط©
...
// ===================== ط§ظ„ظ…ط±ظپظ‚ط§طھ ط§ظ„ظ…ط­ط³ظ†ط© =====================
api.post('/businesses/:bizId/attachments', bizAuthMiddleware(), safeHandler('ط±ظپط¹ ظ…ط±ظپظ‚', async (c) => {
  const bizId = getBizId(c);
  const userId = getUserId(c);
  const body = normalizeBody(await c.req.json());
  if (!body.entityType || !body.entityId || !body.fileName) {
    return c.json({ error: 'entityType, entityId, fileName ظ…ط·ظ„ظˆط¨ط©' }, 400);
  }

  const entityType = String(body.entityType || '').trim();
  const entityId = Number.parseInt(String(body.entityId || ''), 10);
  if (!entityType || !Number.isInteger(entityId) || entityId <= 0) {
    return c.json({ error: 'entityType ط£ظˆ entityId ط؛ظٹط± طµط§ظ„ط­' }, 400);
  }
...
api.delete('/businesses/:bizId/attachments/:id', bizAuthMiddleware(), safeHandler('ط­ط°ظپ ظ…ط±ظپظ‚', async (c) => {
  const id = parseId(c.req.param('id'));
  if (!id) return c.json({ error: 'ظ…ط¹ط±ظ‘ظپ ط§ظ„ظ…ط±ظپظ‚ ط؛ظٹط± طµط§ظ„ط­' }, 400);
  const [existing] = await db.select().from(attachments).where(eq(attachments.id, id));
  if (!existing) return c.json({ error: 'ظ…ط±ظپظ‚ ط؛ظٹط± ظ…ظˆط¬ظˆط¯' }, 404);
  await db.delete(attachments).where(eq(attachments.id, id));
  return c.json({ success: true });
}));

api.get('/businesses/:bizId/attachments-archive-settings', bizAuthMiddleware(), safeHandler('ط¬ظ„ط¨ ط¥ط¹ط¯ط§ط¯ط§طھ ط§ظ„ط£ط±ط´ظپط© ط§ظ„ط¥ظ„ظƒطھط±ظˆظ†ظٹط©', async (c) => {
  const bizId = getBizId(c);
...
api.put('/businesses/:bizId/attachments-archive-settings', bizAuthMiddleware(), safeHandler('ط­ظپط¸ ط¥ط¹ط¯ط§ط¯ط§طھ ط§ظ„ط£ط±ط´ظپط© ط§ظ„ط¥ظ„ظƒطھط±ظˆظ†ظٹط©', async (c) => {
  const bizId = getBizId(c);
  const userId = getUserId(c);
  const body = normalizeBody(await c.req.json());
...
api.post('/businesses/:bizId/attachments-archive-build', bizAuthMiddleware(), safeHandler('ط¥ظ†ط´ط§ط، ط´ط¬ط±ط© ط§ظ„ط£ط±ط´ظپط© طھظ„ظ‚ط§ط¦ظٹط§ظ‹', async (c) => {
  const bizId = getBizId(c);
...
api.post('/businesses/:bizId/attachments-archive-pick-folder', bizAuthMiddleware(), safeHandler('ط§ط®طھظٹط§ط± ظ…ط¬ظ„ط¯ ط§ظ„ط£ط±ط´ظپط© ظ…ظ† ط§ظ„ط¬ظ‡ط§ط²', async (c) => {
  const platform = process.platform;
  if (platform !== 'win32') {
    return c.json({ error: 'ط§ط®طھظٹط§ط± ط§ظ„ظ…ط¬ظ„ط¯ ظ…ظ† ط§ظ„ظ†ط¸ط§ظ… ظ…ط¯ط¹ظˆظ… ط­ط§ظ„ظٹط§ظ‹ ط¹ظ„ظ‰ ظˆظٹظ†ط¯ظˆط² ظپظ‚ط·' }, 400);
  }

  const script = [
    "Add-Type -AssemblyName System.Windows.Forms",
    "$dialog = New-Object System.Windows.Forms.FolderBrowserDialog",
    "$dialog.Description = 'ط§ط®طھط± ظ…ط¬ظ„ط¯ ط§ظ„ط£ط±ط´ظپط©'",
  ...
  try {
    ...
    const msg = String(err?.stderr || err?.message || 'طھط¹ط°ط± ظپطھط­ ظ†ط§ظپط°ط© ط§ط®طھظٹط§ط± ط§ظ„ظ…ط¬ظ„ط¯');
...
api.get('/businesses/:bizId/attachments-archive-fs', bizAuthMiddleware(), safeHandler('ط§ط³طھط¹ط±ط§ط¶ ظ…ط¬ظ„ط¯ط§طھ ط§ظ„ط¬ظ‡ط§ط² ظ„ظ„ط£ط±ط´ظپط©', async (c) => {
  if (process.platform !== 'win32') {
    return c.json({ error: 'ط§ط³طھط¹ط±ط§ط¶ ظ…ظ„ظپط§طھ ط§ظ„ظ†ط¸ط§ظ… ظ…ط¯ط¹ظˆظ… ط­ط§ظ„ظٹط§ظ‹ ط¹ظ„ظ‰ ظˆظٹظ†ط¯ظˆط² ظپظ‚ط·' }, 400);
  }
  ...
  try {
    ...
    return c.json({ error: 'ط§ظ„ظ…ط³ط§ط± ط؛ظٹط± ظ…ظˆط¬ظˆط¯' }, 404);
  }
  if (!s.isDirectory()) return c.json({ error: 'ط§ظ„ظ…ط³ط§ط± ظ„ظٹط³ ظ…ط¬ظ„ط¯ظ‹ط§' }, 400);
...
api.post('/businesses/:bizId/attachments-archive-fs/mkdir', bizAuthMiddleware(), safeHandler('ط¥ظ†ط´ط§ط، ظ…ط¬ظ„ط¯ ط¯ط§ط®ظ„ ظ…ط³طھظƒط´ظپ ط§ظ„ط£ط±ط´ظپط©', async (c) => {
  if (process.platform !== 'win32') {
    return c.json({ error: 'ط¥ظ†ط´ط§ط، ط§ظ„ظ…ط¬ظ„ط¯ ظ…ط¯ط¹ظˆظ… ط­ط§ظ„ظٹط§ظ‹ ط¹ظ„ظ‰ ظˆظٹظ†ط¯ظˆط² ظپظ‚ط·' }, 400);
  }
  ...
  if (!parentPath || !folderName) {
    return c.json({ error: 'path ظˆ name ظ…ط·ظ„ظˆط¨ط§ظ†' }, 400);
  }

  if (folderName.includes('\\') || folderName.includes('/') || folderName.includes('..')) {
    return c.json({ error: 'ط§ط³ظ… ط§ظ„ظ…ط¬ظ„ط¯ ط؛ظٹط± طµط§ظ„ط­' }, 400);
  }
  ...
  try {
    ...
    return c.json({ error: 'ط§ظ„ظ…ط³ط§ط± ط§ظ„ط£ط¨ ط؛ظٹط± ظ…ظˆط¬ظˆط¯' }, 404);
  }
  if (!parentStat.isDirectory()) return c.json({ error: 'ط§ظ„ظ…ط³ط§ط± ط§ظ„ط£ط¨ ظ„ظٹط³ ظ…ط¬ظ„ط¯ظ‹ط§' }, 400);
...
api.get('/businesses/:bizId/attachments-archive-items', bizAuthMiddleware(), safeHandler('ط¬ظ„ط¨ ط¹ظ†ط§طµط± ط£ط±ط´ظپط© ط§ظ„ظ…ط±ظپظ‚ط§طھ', async (c) => {
  const bizId = getBizId(c);
...
api.post('/businesses/:bizId/attachments/:id/rebuild-path', bizAuthMiddleware(), safeHandler('ط¥ط¹ط§ط¯ط© طھظˆظ„ظٹط¯ ظ…ط³ط§ط± ط§ظ„ظ…ط±ظپظ‚', async (c) => {
  const bizId = getBizId(c);
  const id = parseId(c.req.param('id'));
  if (!id) return c.json({ error: 'ظ…ط¹ط±ظ‘ظپ ط§ظ„ظ…ط±ظپظ‚ ط؛ظٹط± طµط§ظ„ط­' }, 400);
  const body = normalizeBody(await c.req.json());
  const [existing] = await db.select().from(attachments).where(eq(attachments.id, id));
  if (!existing) return c.json({ error: 'ط§ظ„ظ…ط±ظپظ‚ ط؛ظٹط± ظ…ظˆط¬ظˆط¯' }, 404);
  if (String(existing.entityType) !== 'voucher') return c.json({ error: 'ط¥ط¹ط§ط¯ط© ط§ظ„طھظˆظ„ظٹط¯ طھط¯ط¹ظ… ظ…ط±ظپظ‚ط§طھ ط§ظ„ط³ظ†ط¯ط§طھ ظپظ‚ط· ط­ط§ظ„ظٹط§ظ‹' }, 400);
  ...
  if (!nextPath) return c.json({ error: 'طھط¹ط°ط± طھط­ط¯ظٹط¯ ط§ظ„ط³ظ†ط¯ ط§ظ„ظ…ط±طھط¨ط· ط¨ظ‡ط°ط§ ط§ظ„ظ…ط±ظپظ‚' }, 400);
