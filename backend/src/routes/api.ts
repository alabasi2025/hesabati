...
// Helper: التحقق من صلاحية المستخدم لإنشاء سند (أدوار + resource vouchers / action create، أو admin)
async function userCanCreateVoucher(userId: number, bizId: number, userRole: string): Promise<boolean> {
  if (userRole === 'admin') return true;
  const urRows = await db.select({ roleId: userRoles.roleId }).from(userRoles)
    .where(and(eq(userRoles.userId, userId), eq(userRoles.businessId, bizId)));
  const roleIds = urRows.map(r => r.roleId);
  if (roleIds.length === 0) return false;
  const permRows = await db.select({ id: rolePermissions.id }).from(rolePermissions)
    .where(and(
      inArray(rolePermissions.roleId, roleIds),
      eq(rolePermissions.resource, 'vouchers'),
      eq(rolePermissions.action, 'create'),
    ))
    .limit(1);
...
api.get('/businesses', safeHandler('جلب قائمة الأعمال', async (c) => {
  ...
  try {
    ...
    for (const row of statsRows as any[]) {
      const bizId = row?.business_id ?? row?.businessId;
      if (bizId != null) {
        statsMap[Number(bizId)] = {
          stations: Number(row?.station_count ?? row?.stationCount ?? 0),
          employees: Number(row?.employee_count ?? row?.employeeCount ?? 0),
          accounts: Number(row?.account_count ?? row?.accountCount ?? 0),
          funds: Number(row?.fund_count ?? row?.fundCount ?? 0),
          suppliers: Number(row?.supplier_count ?? row?.supplierCount ?? 0),
          pendingAccounts: Number(row?.pending_count ?? row?.pendingCount ?? 0),
        };
      }
    }
  } catch (statsErr: any) {
    console.error('جلب إحصائيات الأعمال:', statsErr?.message || statsErr);
    statsMap = {};
  }
  ...
  for (const p of allPartnersDetailed) {
    ...
    if (!partnerDetailMap[bid]) partnerDetailMap[bid] = [];
    partnerDetailMap[bid].push(p);
  }
  
  const defaultStats = { stations: 0, employees: 0, accounts: 0, funds: 0, suppliers: 0, pendingAccounts: 0 };
...
api.post('/businesses/:bizId/vouchers', bizAuthMiddleware(), safeHandler('إضافة سند', async (c) => {
  ...
  const canCreate = await userCanCreateVoucher(userId, bizId, userRole);
  if (!canCreate) return c.json({ error: 'ليس لديك صلاحية إنشاء سندات. راجع إعدادات الأدوار والصلاحيات.' }, 403);
  ...
  // === إلزام وجود الحساب المصدر (الدائن) لسندات التحصيل/الصرف/التحويل/التوريد حتى لا يُنشأ قيد غير متوازن ===
  const needsCreditAccount = ['receipt', 'payment', 'transfer', 'collection', 'delivery'].includes(vType);
  if (needsCreditAccount && !creditAccountId) {
    return c.json({
      error: 'الحساب المصدر (الدائن) مطلوب لهذا النوع من السندات. حدّد حساب المصدر في نوع العملية (source_account_id) أو أرسل fromAccountId.',
    }, 400);
  }
  ...
  // === التحقق من الرصيد الكافي عند الصرف (payment): المصدر هو الحساب الدائن ===
  if (vType === 'payment' && creditAccountId) {
    const balResult = await db.execute(sql`
      SELECT balance FROM account_balances WHERE account_id = ${creditAccountId} AND currency_id = ${currencyId}
    `);
    ...
    if (currentBalance < amount) {
      return c.json({
        error: `الرصيد غير كافٍ للصرف. رصيد الحساب المصدر: ${currentBalance.toLocaleString('ar-SA')}، المطلوب: ${amount.toLocaleString('ar-SA')}`,
      }, 400);
    }
  }
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (!voucherNumber) {
      const seqName = vType === 'receipt' ? 'voucher_receipt_seq'
        : vType === 'payment' ? 'voucher_payment_seq'
        : vType === 'transfer' ? 'voucher_transfer_seq'
        : vType === 'collection' ? 'voucher_collection_seq'
        : vType === 'delivery' ? 'voucher_delivery_seq'
        : 'voucher_receipt_seq';
...
api.post('/businesses/:bizId/operation-types', bizAuthMiddleware(), safeHandler('إضافة نوع عملية', async (c) => {
  ...
  const vType = data.voucherType || data.voucher_type;
  const needsSource = ['receipt', 'payment', 'collection', 'delivery'].includes(vType);
  const hasSource = !!(data.sourceAccountId ?? data.source_account_id ?? data.sourceFundId ?? data.source_fund_id);
  if (needsSource && !hasSource) {
    return c.json({ error: 'أنواع التحصيل/الصرف/التوريد تتطلب تحديد حساب مصدر أو صندوق مصدر (sourceAccountId أو sourceFundId)' }, 400);
  }
...
api.put('/operation-types/:id', safeHandler('تعديل نوع عملية', async (c) => {
  ...
  const [existing] = await db.select().from(operationTypes).where(eq(operationTypes.id, id));
  if (!existing) return c.json({ error: 'نوع العملية غير موجود' }, 404);
  const vType = body.voucherType ?? body.voucher_type ?? (existing as any).voucherType ?? (existing as any).voucher_type;
  const needsSource = ['receipt', 'payment', 'collection', 'delivery'].includes(vType);
  const hasSource = !!(body.sourceAccountId ?? body.source_account_id ?? body.sourceFundId ?? body.source_fund_id ?? (existing as any).sourceAccountId ?? (existing as any).source_account_id ?? (existing as any).sourceFundId ?? (existing as any).source_fund_id);
  if (needsSource && !hasSource) {
    return c.json({ error: 'أنواع التحصيل/الصرف/التوريد تتطلب تحديد حساب مصدر أو صندوق مصدر (sourceAccountId أو sourceFundId)' }, 400);
  }
...
api.get('/businesses/:bizId/sidebar-items', bizAuthMiddleware(), safeHandler('جلب عناصر السايدبار', async (c) => {
  ...
  // استبعاد عناصر شاشات مخصصة وهمية (screen_key = custom-screen-X والشاشة X محذوفة)
  const customScreenIds = new Set(
    (await db.select({ id: screenTemplates.id }).from(screenTemplates).where(eq(screenTemplates.businessId, bizId)))
      .map((r) => r.id)
  );
  const filtered = rows.filter((r) => {
    const key = r.screenKey || '';
    if (!key.startsWith('custom-screen-')) return true;
    const id = Number(key.replace('custom-screen-', ''));
    return Number.isInteger(id) && customScreenIds.has(id);
  });
...
api.get('/businesses/:bizId/users/:userId/sidebar', bizAuthMiddleware(), safeHandler('جلب سايدبار المستخدم', async (c) => {
  ...
  // معرفات الشاشات المخصصة الموجودة فعلاً (لا نعرض عناصر لشاشات محذوفة)
  const customScreenIds = new Set(
    (await db.select({ id: screenTemplates.id }).from(screenTemplates).where(eq(screenTemplates.businessId, bizId)))
      .map((r) => r.id)
  );
  const isPhantomCustomScreen = (screenKey: string | null) => {
    if (!screenKey || !screenKey.startsWith('custom-screen-')) return false;
    const id = Number(screenKey.replace('custom-screen-', ''));
    return !Number.isInteger(id) || !customScreenIds.has(id);
  };
  ...
  // جلب كل عناصر هذا العمل (مع screenKey لاستبعاد الوهمية)
  const allBizItemsRaw = await db.select({
    id: sidebarItems.id,
    ...
    .leftJoin(sidebarSections, eq(sidebarItems.sectionId, sidebarSections.id))
    .where(eq(sidebarSections.businessId, bizId));
  const allBizItems = allBizItemsRaw.filter((item) => !isPhantomCustomScreen(item.screenKey));
  ...
  // إضافة العناصر الجديدة التي لم تُضف بعد لـ config المستخدم (فقط غير الوهمية)
  ...
  const configsFiltered = configs.filter((cfg) => !isPhantomCustomScreen(cfg.screenKey));
  return c.json(configsFiltered);
}));
...
api.put('/businesses/:bizId/users/:userId/sidebar', bizAuthMiddleware(), safeHandler('تحديث سايدبار المستخدم', async (c) => {
  ...
  for (const item of items) {
    if (item.id) {
      const updatePayload: { isVisible?: boolean; customSortOrder?: number; permission?: string } = {
        isVisible: item.isVisible,
        customSortOrder: item.customSortOrder,
      };
      if (item.permission === 'view' || item.permission === 'execute') {
        updatePayload.permission = item.permission;
      }
      await db.update(userSidebarConfig).set(updatePayload).where(eq(userSidebarConfig.id, item.id));
    }
  }
...
api.get('/businesses/:bizId/screens', bizAuthMiddleware(), safeHandler('جلب الشاشات', async (c) => {
  ...
  try {
    ...
    if (msg.includes('does not exist') || msg.includes('screen_templates') || msg.includes('relation')) {
      console.warn('جدول الشاشات المخصصة غير موجود. نفّذ: pnpm run db:migrate أو تأكد من تطبيق migration 0001.');
      return c.json([]);
    }
    throw err;
  }
...
api.post('/businesses/:bizId/screens', bizAuthMiddleware(), safeHandler('إضافة شاشة', async (c) => {
  ...
  const { widgets, addToSidebar, sectionId: _secId, section_id: _secId2, sidebarSortOrder: _sortOrder, ...screenData } = body;
  let created: any;
  try {
    [created] = await db.insert(screenTemplates).values({
      name: screenData.name,
      description: screenData.description ?? null,
      icon: screenData.icon ?? 'dashboard',
      color: screenData.color ?? '#3b82f6',
      layoutConfig: screenData.layoutConfig ?? {},
      templateKey: screenData.templateKey ?? null,
      isSystem: screenData.isSystem === true,
      isActive: screenData.isActive !== false,
      businessId: bizId,
      createdBy: (c.get('user') as any)?.userId ?? null,
    }).returning();
  } catch (insertErr: any) {
    const msg = String(insertErr?.message || insertErr);
    if (msg.includes('does not exist') || msg.includes('screen_templates')) {
      return c.json({
        error: 'جدول الشاشات المخصصة غير موجود. يرجى تطبيق التحديثات على قاعدة البيانات (تنفيذ الملف backend/drizzle/0001_powerful_morlocks.sql).',
      }, 503);
    }
    throw insertErr;
  }
  
  if (!created) return c.json({ error: 'فشل إنشاء الشاشة' }, 500);

  const templateKey = body.templateKey ?? body.template_key ?? null;
  if (templateKey === 'collection_style') {
    try {
      const configPayload = body.collectionStyleConfig ?? body.collection_style_config ?? {};
      await db.insert(screenCollectionStyleConfig).values({
        screenId: created.id,
        tab1Label: configPayload.tab1Label ?? configPayload.tab1_label ?? 'تحصيل',
        tab1Color: configPayload.tab1Color ?? configPayload.tab1_color ?? '#0d9488',
        tab1Icon: configPayload.tab1Icon ?? configPayload.tab1_icon ?? null,
        tab1OperationTypeIds: Array.isArray(configPayload.tab1OperationTypeIds) ? configPayload.tab1OperationTypeIds : (Array.isArray(configPayload.tab1_operation_type_ids) ? configPayload.tab1_operation_type_ids : []),
        tab2Label: configPayload.tab2Label ?? configPayload.tab2_label ?? 'توريد',
        tab2Color: configPayload.tab2Color ?? configPayload.tab2_color ?? '#3b82f6',
        tab2Icon: configPayload.tab2Icon ?? configPayload.tab2_icon ?? null,
        tab2OperationTypeIds: Array.isArray(configPayload.tab2OperationTypeIds) ? configPayload.tab2OperationTypeIds : (Array.isArray(configPayload.tab2_operation_type_ids) ? configPayload.tab2_operation_type_ids : []),
        tab3Label: configPayload.tab3Label ?? configPayload.tab3_label ?? 'السجل',
        tab3Color: configPayload.tab3Color ?? configPayload.tab3_color ?? '#6366f1',
        tab3Icon: configPayload.tab3Icon ?? configPayload.tab3_icon ?? null,
        accountsSectionLabel: configPayload.accountsSectionLabel ?? configPayload.accounts_section_label ?? 'الصناديق',
        tab4Color: configPayload.tab4Color ?? configPayload.tab4_color ?? '#8b5cf6',
        tab4Icon: configPayload.tab4Icon ?? configPayload.tab4_icon ?? null,
        accountIds: Array.isArray(configPayload.accountIds) ? configPayload.accountIds : (Array.isArray(configPayload.account_ids) ? configPayload.account_ids : []),
      });
    } catch (configErr: any) {
      console.warn('إنشاء إعداد الشاشة الثابتة فشل (الشاشة مُنشأة):', configErr?.message || configErr);
    }
  }
  ...
  if (widgets && Array.isArray(widgets)) {
    try {
      ...
      console.warn('إضافة عناصر الشاشة فشلت (الشاشة مُنشأة):', widgetErr?.message || widgetErr);
    }
  }
  ...
  if (addToSidebar) {
    try {
      ...
      if (!sectionId) {
        ...
        if (defaultSection) sectionId = defaultSection.id;
        else {
      ...
      const screenRoute = `/biz/${bizId}/custom-screens?screen=${created.id}`;
      const [sidebarItem] = await db.insert(sidebarItems).values({
        ...
        route: screenRoute,
        sortOrder: body.sidebarSortOrder ?? 99,
        isActive: true,
      }).returning();
      if (sidebarItem) {
        const configs = await db.select().from(userSidebarConfig).where(eq(userSidebarConfig.businessId, bizId));
        let userIds = [...new Set(configs.map((x: any) => x.userId))];
        if (userIds.length === 0) {
          const currentUserId = (c.get('user') as any)?.userId;
          if (currentUserId) userIds.push(currentUserId);
        }
        for (const uid of userIds) {
          await db.insert(userSidebarConfig).values({
            userId: uid, businessId: bizId, sidebarItemId: sidebarItem.id,
            isVisible: true, customSortOrder: sidebarItem.sortOrder ?? 0,
          });
        }
      }
    } catch (sidebarErr: any) {
      console.warn('إضافة الشاشة للقائمة الجانبية فشلت (الشاشة مُنشأة):', sidebarErr?.message || sidebarErr);
    }
  }
...
// صلاحية المستخدم الحالي لشاشة مخصصة (للتطبيق في الواجهة)
api.get('/businesses/:bizId/screens/:screenId/my-permission', bizAuthMiddleware(), safeHandler('جلب صلاحيتي للشاشة', async (c) => {
  const bizId = c.get('bizId') as number;
  const userId = (c.get('user') as any)?.userId as number | undefined;
  ...
  if (!userId) return c.json({ permission: 'execute' });
  const [row] = await db.select({ permission: screenPermissions.permission }).from(screenPermissions)
    .where(and(eq(screenPermissions.screenId, screenId), eq(screenPermissions.userId, userId)));
  const permission = row?.permission === 'none' ? 'none' : (row?.permission === 'view' ? 'view' : 'execute');
  return c.json({ permission });
}));

// ===================== إعداد الشاشة الثابتة (نوع التحصيل/التوريد) =====================
api.get('/businesses/:bizId/screens/:screenId/collection-style-config', bizAuthMiddleware(), safeHandler('جلب إعداد الشاشة الثابتة', async (c) => {
  const bizId = c.get('bizId') as number;
  const screenId = parseId(c.req.param('screenId'));
  if (!screenId) return c.json({ error: 'معرّف الشاشة غير صالح' }, 400);
  const [screen] = await db.select().from(screenTemplates).where(and(eq(screenTemplates.id, screenId), eq(screenTemplates.businessId, bizId)));
  if (!screen) return c.json({ error: 'الشاشة غير موجودة أو لا تنتمي لهذا العمل' }, 404);
  const [config] = await db.select().from(screenCollectionStyleConfig).where(eq(screenCollectionStyleConfig.screenId, screenId));
  if (!config) {
    return c.json({
      tab1Label: 'تحصيل',
      tab1Color: '#0d9488',
  ...
  return c.json({
    tab1Label: config.tab1Label,
    tab1Color: config.tab1Color ?? '#0d9488',
    tab1Icon: config.tab1Icon ?? null,
    tab1OperationTypeIds: Array.isArray(config.tab1OperationTypeIds) ? config.tab1OperationTypeIds : [],
    tab2Label: config.tab2Label,
    tab2Color: config.tab2Color ?? '#3b82f6',
    tab2Icon: config.tab2Icon ?? null,
    tab2OperationTypeIds: Array.isArray(config.tab2OperationTypeIds) ? config.tab2OperationTypeIds : [],
    tab3Label: (config as any).tab3Label ?? 'السجل',
    tab3Color: (config as any).tab3Color ?? '#6366f1',
    tab3Icon: (config as any).tab3Icon ?? null,
    accountsSectionLabel: config.accountsSectionLabel,
    tab4Color: (config as any).tab4Color ?? '#8b5cf6',
    tab4Icon: (config as any).tab4Icon ?? null,
    accountIds: Array.isArray(config.accountIds) ? config.accountIds : [],
  });
}));

api.put('/businesses/:bizId/screens/:screenId/collection-style-config', bizAuthMiddleware(), safeHandler('حفظ إعداد الشاشة الثابتة', async (c) => {
  const bizId = c.get('bizId') as number;
  const screenId = parseId(c.req.param('screenId'));
  if (!screenId) return c.json({ error: 'معرّف الشاشة غير صالح' }, 400);
  const [screen] = await db.select().from(screenTemplates).where(and(eq(screenTemplates.id, screenId), eq(screenTemplates.businessId, bizId)));
  ...
  let body: any;
  try {
    body = normalizeBody(await c.req.json());
  } catch (e: any) {
    return c.json({ error: 'صيغة البيانات غير صحيحة: ' + (e?.message || 'لا يمكن قراءة الطلب') }, 400);
  }
  const tab1Label = body.tab1Label ?? 'تحصيل';
  const tab1Color = body.tab1Color ?? '#0d9488';
  const tab1Icon = body.tab1Icon ?? null;
  const tab1OperationTypeIds = Array.isArray(body.tab1OperationTypeIds) ? body.tab1OperationTypeIds : [];
  const tab2Label = body.tab2Label ?? 'توريد';
  const tab2Color = body.tab2Color ?? '#3b82f6';
  const tab2Icon = body.tab2Icon ?? null;
  const tab2OperationTypeIds = Array.isArray(body.tab2OperationTypeIds) ? body.tab2OperationTypeIds : [];
  const tab3Label = body.tab3Label ?? 'السجل';
  const tab3Color = body.tab3Color ?? '#6366f1';
  const tab3Icon = body.tab3Icon ?? null;
  const accountsSectionLabel = body.accountsSectionLabel ?? 'الصناديق';
  const tab4Color = body.tab4Color ?? '#8b5cf6';
  const tab4Icon = body.tab4Icon ?? null;
  const accountIds = Array.isArray(body.accountIds) ? body.accountIds : [];
  const existing = await db.select().from(screenCollectionStyleConfig).where(eq(screenCollectionStyleConfig.screenId, screenId));
  if (existing.length > 0) {
    const [updated] = await db.update(screenCollectionStyleConfig).set({
      tab1Label,
      tab1Color,
      tab1Icon,
      tab1OperationTypeIds,
      ...
      tab3Color,
      tab3Icon,
      accountsSectionLabel,
      tab4Color,
      tab4Icon,
      accountIds,
      updatedAt: new Date(),
    }).where(eq(screenCollectionStyleConfig.screenId, screenId)).returning();
    return c.json(updated ?? existing[0]);
  }
  const [created] = await db.insert(screenCollectionStyleConfig).values({
    screenId,
    tab1Label,
    tab1Color,
    tab1Icon,
...
api.post('/businesses/:bizId/screens/:screenId/add-to-sidebar', bizAuthMiddleware(), safeHandler('إضافة شاشة للسايدبار', async (c) => {
  ...
  // رابط يفتح صفحة الشاشات المخصصة مع فتح الشاشة مباشرة
  const screenRoute = `/biz/${bizId}/custom-screens?screen=${screenId}`;
...
api.get('/businesses/:bizId/widget-log', bizAuthMiddleware(), safeHandler('جلب سجل العمليات للعنصر', async (c) => {
  ...
  const operationTypeIdsParam = c.req.query('operationTypeIds'); // قائمة أنواع عمليات (لشاشة ثابتة: تبويب1+تبويب2)
  ...
  if (operationTypeIdsParam) {
    const ids = operationTypeIdsParam.split(',').map((s: string) => parseInt(s.trim())).filter((n: number) => !isNaN(n));
    if (ids.length > 0) {
      conditions = sql`${conditions} AND je.operation_type_id IN (${sql.join(ids.map((id: number) => sql`${id}`), sql`, `)})`;
    }
  }
...
// جلب بيانات مراقبة الحسابات (أرصدة حقيقية + آخر حركات) + الصناديق عند تمرير accountIds
api.get('/businesses/:bizId/widget-accounts', bizAuthMiddleware(), safeHandler('جلب بيانات مراقبة الحسابات', async (c) => {
  ...
  const ids = accountIdsParam
    ? accountIdsParam.split(',').map(Number).filter((n: number) => !isNaN(n))
    : [];

  // 1) الحسابات (جدول accounts)
  ...
  const accountRows = Array.isArray(rows) ? rows : (rows as any).rows || [];

  // 2) عند وجود accountIds: إرجاع صناديق (funds) أيضاً بنفس الشكل للمراقبة
  let fundRows: any[] = [];
  if (ids.length > 0) {
    const fundResult = await db.execute(sql`
      SELECT
        f.id,
        f.name,
        'fund' as account_type,
        f.is_active as is_active,
        COALESCE((
          SELECT SUM(CAST(fb.balance AS NUMERIC))
          FROM fund_balances fb WHERE fb.fund_id = f.id
        ), 0) as total_balance,
        (
          SELECT json_agg(bal_row) FROM (
            SELECT fb.id, fb.currency_id, fb.balance, c.code as currency_code, c.symbol as currency_symbol
            FROM fund_balances fb
            LEFT JOIN currencies c ON c.id = fb.currency_id
            WHERE fb.fund_id = f.id
          ) bal_row
        ) as balances,
        NULL::json as last_movements
      FROM funds f
      WHERE f.business_id = ${bizId} AND f.id IN (${sql.join(ids.map((id: number) => sql`${id}`), sql`, `)})
      ORDER BY f.name
    `);
    fundRows = Array.isArray(fundResult) ? fundResult : (fundResult as any).rows || [];
  }

  const resultRows = [...accountRows, ...fundRows];
...
api.get('/businesses/:bizId/exchange-rates/convert', bizAuthMiddleware(), safeHandler('تحويل عملة', async (c) => {
  const bizId = c.get('bizId') as number;
  const fromId = parseInt(c.req.query('from') || '0');
  const toId = parseInt(c.req.query('to') || '0');
  const amountStr = c.req.query('amount') || '1';
  if (!fromId || !toId) return c.json({ error: 'from و to مطلوبان' }, 400);
  if (fromId === toId) return c.json({ rate: 1, convertedAmount: parseFloat(amountStr), fromCurrencyId: fromId, toCurrencyId: toId });
  // جلب أحدث سعر صرف
  ...
  if (!latestRate) {
    ...
    const rate = 1 / parseFloat(String(reverseRate.rate));
    return c.json({ rate, convertedAmount: parseFloat(amountStr) * rate, fromCurrencyId: fromId, toCurrencyId: toId });
  }
  const rate = parseFloat(String(latestRate.rate));
  return c.json({ rate, convertedAmount: parseFloat(amountStr) * rate, fromCurrencyId: fromId, toCurrencyId: toId });
}));
...
api.get('/businesses/:bizId/roles', bizAuthMiddleware(), safeHandler('جلب الأدوار', async (c) => {
  ...
  for (const p of perms) { if (!permMap[p.roleId]) permMap[p.roleId] = []; permMap[p.roleId].push(p); }
...
api.post('/businesses/:bizId/user-roles', bizAuthMiddleware(), safeHandler('تعيين دور لمستخدم', async (c) => {
  const bizId = c.get('bizId') as number;
  const assignedBy = (c.get('user') as any)?.userId;
  const body = normalizeBody(await c.req.json());
...
api.post('/businesses/:bizId/vouchers/:id/reverse', bizAuthMiddleware(), safeHandler('عكس سند', async (c) => {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const vType = original.voucherType;
    const seqName = vType === 'receipt' ? 'voucher_receipt_seq' : vType === 'payment' ? 'voucher_payment_seq' : vType === 'transfer' ? 'voucher_transfer_seq' : vType === 'collection' ? 'voucher_collection_seq' : vType === 'delivery' ? 'voucher_delivery_seq' : 'voucher_receipt_seq';
    const prefix = vType === 'receipt' ? 'RCV' : vType === 'payment' ? 'PAY' : vType === 'transfer' ? 'TRF' : vType === 'collection' ? 'COL' : vType === 'delivery' ? 'DLV' : 'VCH';
    const seqResult = await tx.execute(sql.raw(`SELECT nextval('${seqName}')`));
    const seqRows = Array.isArray(seqResult) ? seqResult : (seqResult as any).rows || [];
    const seqVal = parseInt(String((seqRows[0] as any)?.nextval || 1));
...
// تقرير كشف حساب
api.get('/businesses/:bizId/reports/account-statement/:accountId', bizAuthMiddleware(), safeHandler('كشف حساب', async (c) => {
  const bizId = c.get('bizId') as number;
...
api.get('/businesses/:bizId/reports/trial-balance', bizAuthMiddleware(), safeHandler('ميزان المراجعة', async (c) => {
  const bizId = c.get('bizId') as number;
  const dateFrom = c.req.query('dateFrom');
  const dateTo = c.req.query('dateTo');
