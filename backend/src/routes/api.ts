...
api.get('/businesses/:bizId/users/:userId/sidebar', bizAuthMiddleware(), safeHandler('جلب سايدبار المستخدم', async (c) => {
  ...
  const configs = await db.select({
    id: userSidebarConfig.id, sidebarItemId: userSidebarConfig.sidebarItemId,
    isVisible: userSidebarConfig.isVisible, customSortOrder: userSidebarConfig.customSortOrder,
    // customLabel و customIcon غير موجودة في الـ schema الحالي
    itemLabel: sidebarItems.label, itemIcon: sidebarItems.icon,
    itemRoute: sidebarItems.route, screenKey: sidebarItems.screenKey,
    sectionId: sidebarItems.sectionId, sectionName: sidebarSections.name,
    sectionIcon: sidebarSections.icon, sectionSortOrder: sidebarSections.sortOrder,
  ...
  // إذا لم يكن هناك إعدادات، أنشئ إعدادات افتراضية
  if (configs.length === 0) {
    const allItems = await db.select().from(sidebarItems)
      .leftJoin(sidebarSections, eq(sidebarItems.sectionId, sidebarSections.id))
      .where(eq(sidebarSections.businessId, bizId));
    
    for (const item of allItems) {
      await db.insert(userSidebarConfig).values({
        userId, businessId: bizId, sidebarItemId: (item as any).sidebar_items.id,
        isVisible: true, customSortOrder: (item as any).sidebar_items.sortOrder || 0,
      });
    }
    
    // إعادة الجلب
    const newConfigs = await db.select({
      id: userSidebarConfig.id, sidebarItemId: userSidebarConfig.sidebarItemId,
      isVisible: userSidebarConfig.isVisible, customSortOrder: userSidebarConfig.customSortOrder,
      // customLabel و customIcon غير موجودة في الـ schema الحالي
      itemLabel: sidebarItems.label, itemIcon: sidebarItems.icon,
