...
export const voucherSchema = z.object({
  ...
  operationTypeId: z
    .number()
    .int()
    .positive({ message: "معرّف نوع العملية (القالب) مطلوب" }),
...
export const journalEntrySchema = z.object({
  description: z.string().optional().nullable(),
  entryDate: z
    .union([z.string().min(1, "تاريخ القيد مطلوب"), z.string().optional()])
    .optional(),
...
export const typeSchema = z.object({
  name: z
    ...
    .max(200, "الاسم طويل جداً (حد أقصى 200 حرف)"),
  subTypeKey: z
    .string({
      required_error: "المفتاح مطلوب (subTypeKey)",
      invalid_type_error: "المفتاح يجب أن يكون نصاً",
    })
    .min(1, "المفتاح لا يمكن أن يكون فارغاً")
    .max(100, "المفتاح طويل جداً (حد أقصى 100 حرف)"),
  description: z.string().optional().nullable(),
  ...
  sortOrder: z
    ...
    .optional(),
  isActive: z
    .boolean({ invalid_type_error: "الحالة يجب أن تكون true أو false" })
    .optional(),
});
...
export const journalCategorySchema = z.object({
  ...
  categoryKey: z
    .string({
      required_error: "المفتاح مطلوب",
      invalid_type_error: "المفتاح يجب أن يكون نصاً",
    })
    .min(1, "المفتاح لا يمكن أن يكون فارغاً")
    .max(100, "المفتاح طويل جداً"),
  description: z.string().optional().nullable(),
  icon: z.string().max(100, "الأيقونة طويلة جداً").optional(),
  color: z.string().max(50, "اللون طويل جداً").optional(),
  sortOrder: z
...
export const supplierSchema = z.object({
  name: z
    .string({ required_error: "اسم المورد مطلوب" })
    .min(1, "اسم المورد لا يمكن أن يكون فارغاً")
    .max(200, "اسم المورد طويل جداً"),
  supplierTypeId: z
    .number({
      required_error: "تصنيف المورد مطلوب",
      invalid_type_error: "تصنيف المورد يجب أن يكون رقماً",
    })
    .int()
    .positive("تصنيف المورد مطلوب"),
  ...
  contactPerson: z
    .string()
    .max(200, "جهة الاتصال طويلة جداً")
    .optional()
    .nullable(),
  notes: z.string().optional().nullable(),
  isActive: z.boolean().optional(),
...
export const warehouseSchema = z.object({
  name: z
    ...
    .min(1, "اسم المخزن لا يمكن أن يكون فارغاً")
    .max(200, "اسم المخزن طويل جداً"),
  warehouseType: z.enum(["main", "station"], {
    required_error: "نوع المخزن مطلوب",
    invalid_type_error: "نوع المخزن غير صالح",
  }),
  subType: z
    .string()
    .max(100, "التصنيف الفرعي طويل جداً")
    .optional()
    .nullable(),
  subTypeId: z
    .number({ invalid_type_error: "معرّف التصنيف يجب أن يكون رقماً" })
    .int()
    .positive()
    .optional()
    .nullable(),
  stationId: z
    .number({ invalid_type_error: "معرّف المحطة يجب أن يكون رقماً" })
    .int()
    .positive()
    .optional()
    .nullable(),
...
export const fundSchema = z.object({
  name: z
    .string({ required_error: "اسم الصندوق مطلوب" })
    .min(1, "اسم الصندوق لا يمكن أن يكون فارغاً")
    .max(200, "اسم الصندوق طويل جداً"),
  stationId: z
    .number({ invalid_type_error: "معرّف المحطة يجب أن يكون رقماً" })
    .int()
    .positive()
...
export const pendingAccountSchema = z.object({
  ...
  status: z
    .enum(["pending", "in_progress", "resolved", "written_off"])
    .optional(),
