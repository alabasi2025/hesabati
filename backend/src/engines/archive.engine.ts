/**
 * Archive Engine — منطق الأرشفة الإلكترونية المشترك
 */
import { db } from '../db/index.ts';
import { eq, and } from 'drizzle-orm';
import { funds, accounts, vouchers } from '../db/schema/index.ts';
import { mkdir, readFile, writeFile, stat } from 'node:fs/promises';
import path from 'node:path';

export type ArchiveSettingsPayload = {
  basePath: string;
...
export function getArchiveSettingsDefaults(): ArchiveSettingsPayload {
  return {
    ...
    folderByType: { fund: 'صندوق', bank: 'بنك', exchange: 'صراف', e_wallet: 'محفظة' },
    voucherFolders: { receipt: 'سند قبض', payment: 'سند صرف' },
...
export async function writeArchiveSettings(bizId: number, settings: ArchiveSettingsPayload): Promise<void> {
  const filePath = getArchiveSettingsFilePath(bizId);
...
export async function ensureArchiveTreeForBusiness(
  ...
  const [fundRows, accountRows] = await Promise.all([
    db.select({ name: funds.name }).from(funds).where(eq(funds.businessId, bizId)),
    db.select({ name: accounts.name, accountType: accounts.accountType }).from(accounts).where(eq(accounts.businessId, bizId)),
  ]);

  const byType: Record<'fund' | 'bank' | 'exchange' | 'e_wallet', string[]> = {
    fund: fundRows.map((f: any) => String(f?.name || '')).filter(Boolean),
    bank: [], exchange: [], e_wallet: [],
  };

  for (const acc of accountRows) {
    const type = String(acc?.accountType || '').toLowerCase();
    if (type === 'bank' || type === 'exchange' || type === 'e_wallet' || type === 'fund') {
      byType[type as keyof typeof byType].push(String(acc?.name || '').trim());
    }
  }

  const uniqueDirs = new Set<string>();
  const voucherFolders = [settings.voucherFolders.receipt || 'سند قبض', settings.voucherFolders.payment || 'سند صرف'];
    ...
    for (const name of names) {
      for (const voucherFolder of voucherFolders) {
        for (const level of levels) {
          uniqueDirs.add(path.join(settings.basePath, typeFolder, sanitizePathSegment(name), sanitizePathSegment(voucherFolder), sanitizePathSegment(level)));
        }
      }
    }
  }
...
export async function resolveVoucherArchivePath(
  ...
  const [voucher] = await db
    .select({ id: vouchers.id, voucherType: vouchers.voucherType, fromFundId: vouchers.fromFundId, toFundId: vouchers.toFundId, fromAccountId: vouchers.fromAccountId, toAccountId: vouchers.toAccountId })
    .from(vouchers)
  ...
  let treasuryType: 'fund' | 'bank' | 'exchange' | 'e_wallet' = 'fund';
  let treasuryName = '';

  if (fundId) {
    treasuryType = 'fund';
    const [fund] = await db.select({ name: funds.name }).from(funds).where(and(eq(funds.id, fundId), eq(funds.businessId, bizId))).limit(1);
    treasuryName = String(fund?.name || '');
  } else if (accountId) {
    const [account] = await db.select({ name: accounts.name, accountType: accounts.accountType }).from(accounts).where(and(eq(accounts.id, accountId), eq(accounts.businessId, bizId))).limit(1);
    const t = String(account?.accountType || '').toLowerCase();
    if (t === 'bank' || t === 'exchange' || t === 'e_wallet' || t === 'fund') treasuryType = t as any;
  ...
  return path.join(settings.basePath, sanitizePathSegment(typeFolder), sanitizePathSegment(treasuryName), sanitizePathSegment(voucherFolder), normalizedImportance);
}
