...
import { businesses, accounts, funds, employees } from "./schema-business.ts";
...
export const sequenceCounters = pgTable(
  ...
  (table) => ({
    uniqueCounter: unique("sequence_counters_unique").on(
      table.businessId,
      table.counterType,
      table.entityId,
      table.year,
...
export const accountingTypes = pgTable(
  "accounting_types",
  {
    id: serial("id").primaryKey(),
    businessId: integer("business_id")
  ...
  (table) => ({
    keyUnique: unique("accounting_types_biz_key_unique").on(
      table.businessId,
      table.subTypeKey,
    ),
    seqUnique: unique("accounting_types_biz_seq_unique").on(
      table.businessId,
...
export const accountingSubTypes = pgTable(
  "accounting_sub_types",
  {
    id: serial("id").primaryKey(),
    businessId: integer("business_id")
      .notNull()
      .references(() => businesses.id),
    mainTypeId: integer("main_type_id")
      .notNull()
  ...
  (table) => ({
    keyUnique: unique("accounting_sub_types_biz_key_unique").on(
      table.businessId,
      table.subTypeKey,
    ),
    seqUnique: unique("accounting_sub_types_biz_main_seq_unique").on(
      table.businessId,
      table.mainTypeId,
      table.sequenceNumber,
    ),
...
export const accountSubNatures = pgTable(
  "account_sub_natures",
  {
    id: serial("id").primaryKey(),
    businessId: integer("business_id")
    ...
    requiresAccountNumber: boolean("requires_account_number")
      .notNull()
      .default(false),
    requiresSupplierType: boolean("requires_supplier_type")
      .notNull()
      .default(false),
    supportsCashOperations: boolean("supports_cash_operations")
      .notNull()
      .default(true),
  ...
  (table) => ({
    keyUnique: unique("account_sub_natures_biz_key_unique").on(
      table.businessId,
      table.natureKey,
    ),
    seqUnique: unique("account_sub_natures_biz_seq_unique").on(
      table.businessId,
      table.sequenceNumber,
    ),
  }),
...
export const journalEntryCategories = pgTable(
  ...
  (table) => ({
    keyUnique: unique("journal_entry_cat_biz_key_unique").on(
      table.businessId,
      table.categoryKey,
    ),
    seqUnique: unique("journal_entry_cat_biz_seq_unique").on(
      table.businessId,
      table.sequenceNumber,
    ),
  }),
...
export const supplierTypes = pgTable(
  ...
  (table) => ({
    keyUnique: unique("supplier_types_biz_key_unique").on(
      table.businessId,
      table.subTypeKey,
    ),
    seqUnique: unique("supplier_types_biz_seq_unique").on(
      table.businessId,
      table.sequenceNumber,
    ),
  }),
...
export const inventoryItemTypes = pgTable(
  ...
  (table) => ({
    keyUnique: unique("item_types_biz_key_unique").on(
      table.businessId,
      table.subTypeKey,
    ),
    seqUnique: unique("item_types_biz_seq_unique").on(
      table.businessId,
      table.sequenceNumber,
    ),
  }),
...
export const departments = pgTable(
  ...
  (table) => ({
    codeUnique: unique("departments_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
    seqUnique: unique("departments_biz_seq_unique").on(
      table.businessId,
      table.sequenceNumber,
    ),
  }),
...
export const jobTitles = pgTable(
  ...
  (table) => ({
    seqUnique: unique("job_titles_biz_seq_unique").on(
      table.businessId,
      table.sequenceNumber,
    ),
  }),
...
export const reconciliationItems = pgTable("reconciliation_items", {
  ...
  matchStatus: varchar("match_status", { length: 30 })
    .notNull()
    .default("unmatched_system"),
...
export const purchaseInvoices = pgTable(
  "purchase_invoices",
  {
    ...
    discount: decimal("discount", { precision: 20, scale: 2 })
      .notNull()
      .default("0"),
    ...
    receivedQuantity: decimal("received_quantity", { precision: 20, scale: 4 })
      .notNull()
      .default("0"),
    receivedStatus: varchar("received_status", { length: 20 })
      .notNull()
      .default("pending"),

    sequenceNumber: integer("sequence_number"),
    fullSequenceNumber: varchar("full_sequence_number", { length: 100 }),
    supplierSequenceNumber: varchar("supplier_sequence_number", {
      length: 100,
    }),
  ...
  (table) => ({
    invoiceNumUnique: unique("purchase_invoices_biz_num_unique").on(
      table.businessId,
      table.invoiceNumber,
    ),
  }),
...
export const warehouseOperations = pgTable("warehouse_operations", {
  ...
  operationTypeId: integer("operation_type_id").references(
    () => operationTypes.id,
  ), // القالب المستخدم
  sourceWarehouseId: integer("source_warehouse_id").references(
    () => warehouses.id,
  ), // المخزن المصدر
  destinationWarehouseId: integer("destination_warehouse_id").references(
    () => warehouses.id,
  ), // المخزن الوجهة
...
export const warehouseOperationItems = pgTable("warehouse_operation_items", {
  ...
  operationId: integer("operation_id")
    .notNull()
    .references(() => warehouseOperations.id),
...
export const workflowHistory = pgTable("workflow_history", {
  ...
  transitionId: integer("transition_id").references(
    () => workflowTransitions.id,
  ),
