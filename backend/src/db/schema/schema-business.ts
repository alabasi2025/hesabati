...
export const businessPartners = pgTable("business_partners", {
  ...
  notes: text("notes"),
  defaultCurrencyId: integer("default_currency_id").references(
    () => currencies.id,
  ),
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").notNull().defaultNow(),
...
export const partnerBalances = pgTable(
  ...
  (table) => ({
    partnerCurrencyUnique: uniqueIndex(
      "partner_balances_partner_currency_unique",
    ).on(table.partnerId, table.currencyId),
  }),
);
...
export const stations = pgTable(
  ...
  (table) => ({
    codeUnique: unique("stations_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
    seqUnique: unique("stations_biz_seq_unique").on(
      table.businessId,
...
export const employees = pgTable("employees", {
  ...
  monthlyAllowance: decimal("monthly_allowance", {
    precision: 15,
    scale: 2,
  }).default("0"),
...
export const employeeBalances = pgTable(
  "employee_balances",
  {
    id: serial("id").primaryKey(),
  ...
  (table) => ({
    employeeCurrencyUnique: uniqueIndex(
      "employee_balances_employee_currency_unique",
    ).on(table.employeeId, table.currencyId),
  }),
);
...
export const accounts = pgTable(
  "accounts",
  {
    ...
    canBeDebitedByReceipt: boolean("can_be_debited_by_receipt")
      .notNull()
      .default(true),
    ...
    receivesFromStations: boolean("receives_from_stations")
      .notNull()
      .default(false),
    isActive: boolean("is_active").notNull().default(true),
    notes: text("notes"),
  ...
  (table) => ({
    codeUnique: unique("accounts_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
...
export const accountBalances = pgTable(
  ...
  (table) => ({
    accountCurrencyUnique: uniqueIndex(
      "account_balances_account_currency_unique",
    ).on(table.accountId, table.currencyId),
  }),
);
...
export const employeeBillingAccounts = pgTable("employee_billing_accounts", {
  ...
  billingSystemId: integer("billing_system_id").references(
    () => billingSystemsConfig.id,
  ),
...
export const billingSystemsConfig = pgTable(
  "billing_systems_config",
  {
    ...
    color: varchar("color", { length: 20 }).default("#3b82f6"),
    stationMode: varchar("station_mode", { length: 20 })
      .notNull()
      .default("per_station"),
    stationIds: jsonb("station_ids").$type<number[]>().default([]),
    supportedMethodIds: jsonb("supported_method_ids")
      .$type<number[]>()
      .default([]),
  ...
  (table) => ({
    keyUnique: unique("billing_systems_config_biz_key_unique").on(
      table.businessId,
      table.systemKey,
    ),
  }),
...
export const wallets = pgTable(
  ...
  (table) => ({
    codeUnique: unique("wallets_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
  }),
...
export const walletBalances = pgTable(
  ...
  (table) => ({
    walletCurrencyUnique: uniqueIndex(
      "wallet_balances_wallet_currency_unique",
    ).on(table.walletId, table.currencyId),
  }),
);
...
export const exchanges = pgTable(
  ...
  (table) => ({
    codeUnique: unique("exchanges_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
  }),
...
export const exchangeBalances = pgTable(
  ...
  (table) => ({
    exchangeCurrencyUnique: uniqueIndex(
      "exchange_balances_exchange_currency_unique",
    ).on(table.exchangeId, table.currencyId),
  }),
);
...
export const banks = pgTable(
  ...
  (table) => ({
    codeUnique: unique("banks_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
  }),
...
export const bankBalances = pgTable(
  ...
  (table) => ({
    bankCurrencyUnique: uniqueIndex("bank_balances_bank_currency_unique").on(
      table.bankId,
      table.currencyId,
    ),
  }),
...
export const funds = pgTable(
  ...
  (table) => ({
    codeUnique: unique("funds_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
  }),
...
export const fundBalances = pgTable(
  "fund_balances",
  {
    id: serial("id").primaryKey(),
  ...
  (table) => ({
    fundCurrencyUnique: uniqueIndex("fund_balances_fund_currency_unique").on(
      table.fundId,
      table.currencyId,
    ),
  }),
...
export const accountCurrencies = pgTable(
  ...
  (table) => ({
    accountCurrencyUnique: uniqueIndex("account_currencies_unique").on(
      table.accountId,
      table.currencyId,
    ),
  }),
...
export const fiscalYears = pgTable(
  ...
  (table) => ({
    bizYearUnique: uniqueIndex("fiscal_years_biz_year_unique").on(
      table.businessId,
      table.year,
    ),
  }),
...
export const fiscalPeriods = pgTable(
  ...
  (table) => ({
    yearMonthUnique: uniqueIndex("fiscal_periods_year_month_unique").on(
      table.fiscalYearId,
      table.month,
    ),
  }),
