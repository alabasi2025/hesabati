...
export const operationCategories = pgTable(
  ...
  (table) => ({
    keyUnique: unique("op_categories_biz_key_unique").on(
      table.businessId,
      table.categoryKey,
    ),
    codeUnique: unique("op_categories_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
    seqUnique: unique("op_categories_biz_seq_unique").on(
      table.businessId,
...
export const operationTypes = pgTable(
  "operation_types",
  {
    ...
    categoryId: integer("category_id")
      .notNull()
      .references(() => operationCategories.id),
    sequenceNumber: integer("sequence_number"),
    code: varchar("code", { length: 30 }),
  ...
  (table) => ({
    codeUnique: unique("operation_types_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
    seqUnique: unique("operation_types_biz_cat_seq_unique").on(
      table.businessId,
      table.categoryId,
      table.sequenceNumber,
    ),
...
export const operationTypeAccounts = pgTable("operation_type_accounts", {
  ...
  employeeBillingAccountId: integer("employee_billing_account_id").references(
    () => employeeBillingAccounts.id,
  ),
...
export const journalEntries = pgTable("journal_entries", {
  ...
  totalDebit: decimal("total_debit", { precision: 20, scale: 2 })
    .notNull()
    .default("0"),
  totalCredit: decimal("total_credit", { precision: 20, scale: 2 })
    .notNull()
    .default("0"),
...
export const userSidebarConfig = pgTable("user_sidebar_config", {
  ...
  businessId: integer("business_id")
    .notNull()
    .references(() => businesses.id),
  sidebarItemId: integer("sidebar_item_id")
    .notNull()
...
export const customScreenConfig = pgTable("custom_screen_config", {
  ...
  tab1Icon: varchar("tab1_icon", { length: 100 })
    .notNull()
    .default("arrow_downward"),
  tab1Color: varchar("tab1_color", { length: 20 }).notNull().default("#22c55e"),
  tab1OperationTypeIds: jsonb("tab1_operation_type_ids")
    .$type<number[]>()
    .default([]),
  // التبويب الثاني (مثلاً عمليات الصرف)
  tab2Label: varchar("tab2_label", { length: 200 }).notNull().default("توريد"),
  tab2Icon: varchar("tab2_icon", { length: 100 })
    .notNull()
    .default("arrow_upward"),
  tab2Color: varchar("tab2_color", { length: 20 }).notNull().default("#ef4444"),
  tab2OperationTypeIds: jsonb("tab2_operation_type_ids")
    .$type<number[]>()
    .default([]),
  // تبويب السجل
  historyLabel: varchar("history_label", { length: 200 })
    .notNull()
    .default("السجل"),
  historyIcon: varchar("history_icon", { length: 100 })
    .notNull()
    .default("history"),
  historyColor: varchar("history_color", { length: 20 })
    .notNull()
    .default("#6366f1"),
  // قسم المراقبة (صناديق/محافظ/بنوك...)
  accountsSectionLabel: varchar("accounts_section_label", { length: 200 })
    .notNull()
    .default("الصناديق"),
  accountsIcon: varchar("accounts_icon", { length: 100 })
    .notNull()
    .default("savings"),
  accountsColor: varchar("accounts_color", { length: 20 })
    .notNull()
    .default("#10b981"),
