...
export const supplierBalances = pgTable(
  ...
  (table) => ({
    supplierCurrencyUnique: uniqueIndex(
      "supplier_balances_supplier_currency_unique",
    ).on(table.supplierId, table.currencyId),
  }),
);
...
export const warehouses = pgTable(
  ...
  (table) => ({
    codeUnique: unique("warehouses_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
    seqUnique: unique("warehouses_biz_seq_unique").on(
      table.businessId,
...
export const warehouseBalances = pgTable(
  ...
  (table) => ({
    warehouseCurrencyUnique: uniqueIndex(
      "warehouse_balances_warehouse_currency_unique",
    ).on(table.warehouseId, table.currencyId),
  }),
);
...
export const inventoryItems = pgTable(
  ...
  (table) => ({
    codeUnique: unique("inventory_items_biz_code_unique").on(
      table.businessId,
      table.code,
    ),
  }),
...
export const inventoryStock = pgTable("inventory_stock", {
  ...
  costingMethod: varchar("costing_method", { length: 20 }).default(
    "weighted_avg",
  ), // weighted_avg | fifo | lifo
  costLayers: jsonb("cost_layers")
    .$type<Array<{ qty: number; unitCost: number; date: string }>>()
    .default([]), // طبقات التكلفة لـ FIFO/LIFO
  currencyId: integer("currency_id").references(() => currencies.id),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
