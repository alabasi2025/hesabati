...
async function seedSidebar() {
  ...
  for (const biz of businesses) {
    ...
    // --- الأقسام (13 قسم = 13 وحدة حسب MODULES.md) ---
    ...
    console.log(`  ✅ أقسام: 1..13 (${sec1.id}..${sec13.id})`);
    ...
    if (bizType === 'stations') {
      mainItems.push(
        ...
        { sectionId: sec2.id, screenKey: 'summary', label: 'ملخص الأعمال', icon: 'summarize', route: '/biz/{bizId}/summary', sortOrder: 6 },
      );
