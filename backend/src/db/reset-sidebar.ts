...
async function resetSidebar() {
  ...
  // ضمان وجود عمود color في الجداول
  console.log("🔧 التحقق من أعمدة color...");
  await client`ALTER TABLE sidebar_sections ADD COLUMN IF NOT EXISTS color varchar(30) DEFAULT '#6366f1'`;
  await client`ALTER TABLE sidebar_items ADD COLUMN IF NOT EXISTS color varchar(30)`;
  console.log("✅ أعمدة color جاهزة");
  ...
  for (const biz of businesses) {
    ...
    const bizType =
      biz.id === 1 ? "stations" : biz.id === 2 ? "single_station" : "personal";
    ...
    const [sec13] = await db
      ...
      .values({
        businessId: biz.id,
        name: "13. الموظفين والرواتب",
        icon: "groups",
        color: "#22c55e",
        sortOrder: 13,
      })
      .returning();
    const [sec14] = await db
      .insert(schema.sidebarSections)
      .values({
        businessId: biz.id,
        name: "14. الميزانية والمصروفات",
        icon: "account_balance_wallet",
        color: "#d946ef",
        sortOrder: 14,
      })
