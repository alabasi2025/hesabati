...
async function seed() {
  ...
  const [b1] = await db
    ...
    .values({
      ...
      code: "biz-01",
      description:
        "شراكة المحطات الأربع مع محمد المراني - الدهمية، الصبالية وجمال، غليل، الساحل الغربي",
      icon: "bolt",
      color: "#f59e0b",
      sortOrder: 1,
    })
  ...
  const [b2] = await db
    ...
    .values({
      name: "محطة معبر",
      code: "biz-02",
      description: "شراكة محطة معبر مع عمر إسحاق وإبراهيم نجم الدين",
      icon: "flash_on",
      color: "#22c55e",
      sortOrder: 2,
    })
  ...
  const [b3] = await db
    ...
    .values({
      name: "أعمال شخصية",
      code: "biz-03",
      description: "حسابات المالك الشخصية وأعماله الخاصة",
      icon: "account_circle",
      color: "#6366f1",
      sortOrder: 3,
    })
  ...
  for (const biz of [b1, b2, b3]) {
    await db.insert(schema.accountSubNatures).values(
      systemNatures.map((n, idx) => ({
        ...
        requiresEmployee:
          "requiresEmployee" in n && n.requiresEmployee === true,
        requiresProvider:
          "requiresProvider" in n && n.requiresProvider === true,
        requiresAccountNumber:
          "requiresAccountNumber" in n && n.requiresAccountNumber === true,
        requiresSupplierType:
          "requiresSupplierType" in n && n.requiresSupplierType === true,
        supportsCashOperations: true,
  ...
  const createLinkedAccount = async (
    ...
    accountType: NonNullable<
      (typeof schema.accounts.$inferInsert)["accountType"]
    >,
    ...
    const [subNature] = accountSubNatureId
      ? await db
          .select({ natureKey: schema.accountSubNatures.natureKey })
          .from(schema.accountSubNatures)
    ...
    const { code: generatedCode, sequenceNumber: generatedSeq } =
      await generateLeafAccountCode(businessId, natureKey, db as any);
  ...
  const partnerCtrlB1 = await createLinkedAccount(
    b1.id,
    "حسابات الشركاء",
    "partner",
  ...
  for (const pt of partnerSeedData) {
    ...
    // إنشاء حساب تحليلي خاص بالشريك (XXX-YYY-ZZZ)
    const partnerAccount = await createLinkedAccount(
      pt.businessId,
      pt.fullName,
      "partner",
  ...
  const [s1] = await db
    ...
    .values({
      businessId: b1.id,
      name: "الدهمية",
      code: generateItemCode("STN", seq1),
      location: "الدهمية",
      billingSystems: ["moghrabi_v1", "support_fund", "prepaid"],
      sequenceNumber: seq1,
    })
    .returning();
  ...
  const [s2] = await db
    ...
    .values({
      businessId: b1.id,
      name: "الصبالية وجمال",
      code: generateItemCode("STN", seq2),
      location: "الصبالية وجمال",
      billingSystems: ["moghrabi_v2", "support_fund", "prepaid"],
      sequenceNumber: seq2,
    })
    .returning();
  ...
  const [s3] = await db
    ...
    .values({
      businessId: b1.id,
      name: "غليل",
      code: generateItemCode("STN", seq3),
      location: "غليل",
      billingSystems: ["moghrabi_v3", "support_fund", "prepaid"],
      sequenceNumber: seq3,
    })
    .returning();
  ...
  const [s4] = await db
    ...
    .values({
      businessId: b1.id,
      name: "الساحل الغربي",
      code: generateItemCode("STN", seq4),
      location: "الساحل الغربي",
      billingSystems: ["support_fund_west"],
      hasEmployees: false,
      notes: "نقطة مولعة من محطة الشريك",
      sequenceNumber: seq4,
    })
    .returning();
  ...
  const [s5] = await db
    ...
    .values({
      businessId: b1.id,
      name: "الإدارة",
      code: generateItemCode("STN", seq5),
      location: "المكتب الرئيسي",
      billingSystems: [],
      notes: "المكتب الإداري",
      sequenceNumber: seq5,
    })
    .returning();
  ...
  const [s6] = await db
    ...
    .values({
      businessId: b2.id,
      name: "محطة معبر",
      code: generateItemCode("STN", seqMabar),
      location: "معبر",
      billingSystems: ["moghrabi_v3"],
      sequenceNumber: seqMabar,
    })
    .returning();
  console.log("✅ المحطات");
  ...
  const empCtrlB1 = await createLinkedAccount(
    b1.id,
    "حسابات الموظفين",
    "employee",
  ...
  const employeeSeedData: {
    ...
    // الإدارة
    {
      ...
      ctrl: empCtrlB1,
      fullName: "علي الصعدي",
      jobTitle: "محاسب ومفوتر",
      stationId: s5.id,
      department: "الإدارة",
      salary: "300000",
      salaryCurrency: "YER",
      monthlyAllowance: "3000",
      notes: "محاسب + مفوتر للمحطات",
    },
    // الدهمية
    {
      businessId: b1.id,
      ctrl: empCtrlB1,
      fullName: "رايد العباسي",
      jobTitle: "مدير المحطة",
      stationId: s1.id,
      department: "الدهمية",
      salary: "280000",
    },
    ...
    // الصبالية وجمال
    {
      businessId: b1.id,
      ctrl: empCtrlB1,
      fullName: "علي المجهلي",
      jobTitle: "مدير المحطة",
      stationId: s2.id,
      department: "الصبالية وجمال",
      salary: "250000",
    },
    {
      businessId: b1.id,
      ctrl: empCtrlB1,
      fullName: "خالد أبو الرجال",
      stationId: s2.id,
      department: "الصبالية وجمال",
      salary: "200000",
    },
    ...
    // خارجي
    {
      businessId: b1.id,
      ctrl: empCtrlB1,
      fullName: "قايد العباسي",
      jobTitle: "متابعة مواقع فورجي وسنترال",
      department: "خارجي",
      salary: "0",
      notes: "يفوتر ويتحصل شهرياً ويودع لحسابات المالك",
    },
    // معبر
    {
      businessId: b2.id,
      ctrl: empCtrlB2,
      fullName: "حسن المعبري",
      jobTitle: "مدير محطة",
      stationId: s6.id,
      department: "معبر",
      salary: "180000",
  ...
  for (const emp of employeeSeedData) {
    ...
    // إنشاء حساب تحليلي خاص بالموظف (XXX-YYY-ZZZ)
    const empAccount = await createLinkedAccount(
      emp.businessId,
      emp.fullName,
      "employee",
  ...
  for (const row of accountSeedRows) {
    const natureId = row.accountType
  ...
  const supTypesB1: SupTypeDef[] = [
    ...
    {
      key: "electric",
      name: "موردو العدادات والكهربائيات",
      icon: "electrical_services",
      color: "#eab308",
    },
    {
      key: "batteries",
      name: "موردو البطاريات",
      icon: "battery_charging_full",
      color: "#22c55e",
    },
    {
      key: "screens",
      name: "موردو الشاشات والقواطع",
      icon: "computer",
      color: "#3b82f6",
    },
    {
      key: "utility",
      name: "جهات العائد الشهري",
      icon: "electric_bolt",
      color: "#8b5cf6",
    },
  ];
  ...
  const supTypeMapB1 = new Map<
    string,
    Awaited<ReturnType<typeof createSupplierType>>
  >();
  ...
  const supplierSeedData: {
    businessId: number;
    typeKey: string;
    typeMap: typeof supTypeMapB1;
    name: string;
  }[] = [
    {
      businessId: b1.id,
      typeMap: supTypeMapB1,
      typeKey: "diesel",
      name: "محمود الحجة",
    },
    {
    ...
    },
    {
      ...
      typeMap: supTypeMapB1,
      typeKey: "oil",
      name: "الحجاجي (إيني أجيب)",
    },
    {
      businessId: b1.id,
      typeMap: supTypeMapB1,
      typeKey: "oil",
      name: "أحمد هادي (بيل ون)",
    },
    {
      businessId: b1.id,
      typeMap: supTypeMapB1,
  ...
  for (const sup of supplierSeedData) {
    ...
    // إنشاء حساب تحليلي خاص بالمورد (XXX-YYY-ZZZ)
    const supAccount = await createLinkedAccount(
      sup.businessId,
      sup.name,
      "supplier",
  ...
  const whsCtrlB1 = await createLinkedAccount(
    b1.id,
    "حسابات المخازن",
    "warehouse",
  ...
  const warehouseSeedData = [
    ...
    },
    {
      ...
      ctrl: whsCtrlB1,
      name: "مخزن الدهمية",
      warehouseType: "station",
      stationId: s1.id,
    },
    {
      businessId: b1.id,
      ctrl: whsCtrlB1,
      name: "مخزن الصبالية وجمال",
      warehouseType: "station",
      stationId: s2.id,
    },
    {
  ...
  for (const whs of warehouseSeedData) {
    ...
    // إنشاء حساب تحليلي خاص بالمخزن (XXX-YYY-ZZZ)
    const whsAccount = await createLinkedAccount(
      whs.businessId,
      whs.name,
      "warehouse",
  ...
  const cusCtrlB1 = await createLinkedAccount(
    b1.id,
    "حسابات العهد",
    "custody",
  ...
  const custodySeedData = [
    {
      businessId: b1.id,
      ctrl: cusCtrlB1,
      partyName: "أكرم العباسي",
      partyType: "employee",
      custodyType: "permanent",
      contentType: "cash",
      description: "عهدة صيانة محطة الدهمية",
    },
    {
      businessId: b1.id,
      ctrl: cusCtrlB1,
      partyName: "كمال العباسي",
      partyType: "employee",
      custodyType: "permanent",
      contentType: "cash",
      description: "عهدة مشتريات عامة",
    },
  ];
  ...
  for (const cus of custodySeedData) {
    ...
    // إنشاء حساب تحليلي خاص بالعهدة (XXX-YYY-ZZZ)
    const cusAccount = await createLinkedAccount(
      cus.businessId,
      `عهدة - ${cus.partyName}`,
      "custody",
  ...
  const bdgCtrlB1 = await createLinkedAccount(
    b1.id,
    "حسابات الميزانية",
    "budget",
  ...
  const budgetSeedData = [
    ...
    },
    {
      businessId: b1.id,
      ctrl: bdgCtrlB1,
      name: "ميزانية الرواتب السنوية",
      amount: "30000000",
      expenseType: "annual" as const,
      year: 2025,
    },
  ];
  for (const bdg of budgetSeedData) {
    // إنشاء حساب تحليلي خاص ببند الميزانية (XXX-YYY-ZZZ)
    const bdgAccount = await createLinkedAccount(
      bdg.businessId,
      bdg.name,
      "budget",
  ...
  // ===================== أصناف المخزون =====================
  const itemSeq1 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq2 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq3 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq4 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq5 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq6 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq7 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq8 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq9 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  const itemSeq10 = await getNextSequence(
    b1.id,
    "inventory_item",
    0,
    0,
  ...
  await db.insert(schema.inventoryItems).values([
    {
      businessId: b1.id,
      name: "ديزل",
      code: generateItemCode("ITM", itemSeq1),
      category: "وقود",
      unit: "لتر",
    },
    {
      businessId: b1.id,
      name: "زيت مولد - لوتس",
      code: generateItemCode("ITM", itemSeq2),
      category: "زيوت",
      unit: "برميل",
    },
    {
      businessId: b1.id,
      name: "زيت مولد - إيني أجيب",
      code: generateItemCode("ITM", itemSeq3),
      category: "زيوت",
      unit: "برميل",
    },
    {
      businessId: b1.id,
      name: "زيت مولد - بيل ون",
      code: generateItemCode("ITM", itemSeq4),
      category: "زيوت",
      unit: "برميل",
    },
    {
      businessId: b1.id,
      name: "فلتر زيت",
      code: generateItemCode("ITM", itemSeq5),
      category: "فلاتر",
      unit: "حبة",
    },
    {
      businessId: b1.id,
      name: "فلتر هواء",
      code: generateItemCode("ITM", itemSeq6),
      category: "فلاتر",
      unit: "حبة",
    },
    {
      businessId: b1.id,
      name: "عداد توماس",
      code: generateItemCode("ITM", itemSeq7),
      category: "عدادات",
      unit: "حبة",
    },
    {
      businessId: b1.id,
      name: "بطارية مولد",
      code: generateItemCode("ITM", itemSeq8),
      category: "بطاريات",
      unit: "حبة",
    },
    {
      businessId: b1.id,
      name: "كيبل كهربائي",
      code: generateItemCode("ITM", itemSeq9),
    ...
    },
    {
      businessId: b1.id,
      name: "قاطع كهربائي",
      code: generateItemCode("ITM", itemSeq10),
      category: "مواد كهربائية",
      unit: "حبة",
    },
  ]);
  ...
  const pendingItems = [
    {
      ...
      description: "صندوق مخلوط فيه عجز - يحتاج تصفية كاملة",
      status: "pending",
    },
    {
      businessId: b1.id,
      personOrEntity: "أمجد الصلوي",
      description: "حساب العدادات والمواد الكهربائية فيه شعبطة كبيرة",
      status: "pending",
    },
    {
      businessId: b1.id,
      personOrEntity: "المهندس محمد حسن",
      description: "شاشات وقواطع دمج - له سنة ما كمل",
      status: "pending",
    },
  ];
  ...
  const intermediaryAccounts = [
    {
      ...
      description: "فواتير تم تسجيلها وبانتظار إدخال المخزن",
    },
    {
      name: "تحويلات مخزنية قيد الاستلام",
      description: "تحويلات تم إرسالها وبانتظار الاستلام",
    },
    {
      name: "طلبات توريد قيد المعالجة",
      description: "طلبات توريد بانتظار وصول البضاعة",
    },
  ];
  ...
  // ===================== أسعار الصرف الافتراضية =====================
  {
    ...
    for (const biz of [b1, b2, b3]) {
      await db.insert(schema.exchangeRates).values([
        {
          businessId: biz.id,
          fromCurrencyId: sarCurrencyId,
          toCurrencyId: yerCurrencyId,
          rate: "140",
          effectiveDate: today,
          source: "seed",
        },
    ...
    const leafAccounts = await db
      .select({
        ...
        businessId: schema.accounts.businessId,
      })
      .from(schema.accounts)
      .where(eq(schema.accounts.isLeafAccount, true));

    const accountCurrencyRows: {
      accountId: number;
      currencyId: number;
      isDefault: boolean;
    }[] = [];
    ...
    console.log(
      "✅ ربط العملات الثلاث بالحسابات (" +
        accountCurrencyRows.length +
        " سجل = " +
        leafAccounts.length +
        " حساب × 3 عملات)",
    );
  ...
  const billingSystems = [
    {
      name: "المغربي نسخة 1 (الدهمية)",
      systemKey: "moghrabi_v1",
      icon: "receipt",
      color: "#10b981",
      stationMode: "per_station",
      sortOrder: 1,
    },
    {
      name: "المغربي نسخة 2 (الصبالية وجمال)",
      systemKey: "moghrabi_v2",
      icon: "receipt",
      color: "#059669",
      stationMode: "per_station",
      sortOrder: 2,
    ...
    {
      name: "المغربي نسخة 3 (غليل)",
      systemKey: "moghrabi_v3",
      icon: "receipt",
      color: "#047857",
      stationMode: "per_station",
      sortOrder: 3,
    ...
    {
      name: "صندوق الدعم - الساحل الغربي",
      systemKey: "support_fund_west",
      icon: "support",
      color: "#2563eb",
      stationMode: "per_station",
      sortOrder: 5,
    },
    {
      ...
      icon: "credit_card",
      color: "#8b5cf6",
      stationMode: "multi_station",
      sortOrder: 6,
    },
  ];
  const [billingNature] = await db
    .select({ id: schema.accountSubNatures.id })
    .from(schema.accountSubNatures)
  ...
  for (const bsys of billingSystems) {
    const { code: bCode, sequenceNumber: bSeq } = await generateLeafAccountCode(
      b1.id,
      "billing",
      db as any,
    );
    const [bAcc] = await db
      ...
      .returning();
    const [bCfg] = await db
      .insert(schema.billingSystemsConfig)
      .values({
        businessId: b1.id,
        ...bsys,
        accountId: bAcc.id,
  ...
  const billingConfigMap = Object.fromEntries(
    billingConfigs.map((bc) => [bc.systemKey, bc.id]),
  );
  const billingSystemAccountMap = Object.fromEntries(
    billingConfigs.map((bc) => [bc.systemKey, bc.accountId]),
  );
  ...
  await db.insert(schema.billingAccountTypes).values([
    {
      businessId: b1.id,
      name: "تحصيل نقدي بالجوال",
      description: "تحصيل الفواتير نقداً عبر الجوال",
    },
    {
      businessId: b1.id,
      name: "تحصيل إسناد يدوي",
      description: "تحصيل الفواتير بالإسناد اليدوي",
    },
    {
      businessId: b1.id,
      name: "سداد إلكتروني",
      description: "سداد الفواتير إلكترونياً",
    },
  ]);
  ...
  const BILLING_SYSTEM_LABELS: Record<string, string> = {
    ...
    support_fund_west: "صندوق الدعم - الساحل",
  ...
  const allEmps = await db
    ...
    .where(eq(schema.employees.businessId, b1.id));
  ...
  const missingFunds = await db
    .select()
    .from(schema.funds)
    .where(isNull(schema.funds.accountId));
  ...
  if (missingBillingAccounts.length > 0) {
    ...
    const sysConfigs = await db
      .select({
        id: schema.billingSystemsConfig.id,
        accountId: schema.billingSystemsConfig.accountId,
      })
    ...
    console.log(
      `✅ ربط ${missingBillingAccounts.length} حساب فوترة بحسابات تحكم أنظمتها`,
    );
  }
  ...
  for (const biz of [b1, b2, b3]) {
    ...
    const [sec1] = await db
      .insert(schema.sidebarSections)
      .values({
        businessId: biz.id,
        name: "1. المستخدمون والصلاحيات",
    ...
    const [sec9] = await db
      ...
      .returning();
    const [sec10] = await db
      .insert(schema.sidebarSections)
      .values({
        businessId: biz.id,
        name: "10. بناء الواجهات",
        icon: "space_dashboard",
        sortOrder: 10,
      })
      .returning();
    const [sec11] = await db
      .insert(schema.sidebarSections)
      .values({
        businessId: biz.id,
        name: "11. العملات وأسعار الصرف",
        icon: "currency_exchange",
        sortOrder: 11,
      })
      .returning();
    const [sec12] = await db
      .insert(schema.sidebarSections)
      .values({
        businessId: biz.id,
        name: "12. المعلقات والتصفيات",
        icon: "balance",
        sortOrder: 12,
      })
      .returning();
    const [sec13] = await db
      .insert(schema.sidebarSections)
      .values({
        businessId: biz.id,
        name: "13. الرواتب والميزانية",
        icon: "payments",
        sortOrder: 13,
      })
      .returning();
    ...
    await db.insert(schema.sidebarItems).values([
      {
        ...
        icon: "admin_panel_settings",
        route: "/biz/{bizId}/roles",
        sortOrder: 1,
      },
      {
        sectionId: sec1.id,
        screenKey: "sidebar_settings",
        label: "إعدادات التبويب",
        icon: "tune",
        route: "/biz/{bizId}/sidebar-settings",
        sortOrder: 2,
      },
    ]);
    ...
    const accountItems: any[] = [
      ...
      {
        sectionId: sec3.id,
        screenKey: "analytical_accounts",
        label: "الحسابات التحليلية",
        icon: "account_tree",
        route: "/biz/{bizId}/analytical-accounts",
        sortOrder: 3,
      },
    ];
    if (bizType === "stations" || bizType === "single_station") {
      accountItems.push(
        ...
        },
        {
          ...
          label: "البنوك",
          icon: "account_balance",
          route: "/biz/{bizId}/banks",
          sortOrder: 5,
        },
        {
          sectionId: sec3.id,
          screenKey: "exchangers",
          label: "الصرافين",
          icon: "currency_exchange",
          route: "/biz/{bizId}/exchangers",
          sortOrder: 5,
        },
        {
          sectionId: sec3.id,
    ...
    await db.insert(schema.sidebarItems).values([
      {
        sectionId: sec5.id,
        screenKey: "operation_categories",
        label: "أصناف العمليات",
        icon: "folder_special",
        route: "/biz/{bizId}/operation-categories",
        sortOrder: 1,
      },
      {
        sectionId: sec5.id,
        screenKey: "operation_types",
        label: "أنواع العمليات",
        icon: "category",
        route: "/biz/{bizId}/operation-types",
        sortOrder: 2,
      },
      {
        sectionId: sec5.id,
      ...
      },
      {
        ...
        label: "الأقسام",
        icon: "groups",
        route: "/biz/{bizId}/departments",
        sortOrder: 4,
      },
      {
        sectionId: sec5.id,
        screenKey: "job_titles",
        label: "المسميات الوظيفية",
        icon: "badge",
        route: "/biz/{bizId}/job-titles",
        sortOrder: 5,
      },
    ]);
    ...
    await db.insert(schema.sidebarItems).values([
      {
        ...
        label: "المخزن",
        icon: "warehouse",
        route: "/biz/{bizId}/warehouse",
        sortOrder: 1,
      },
      {
        sectionId: sec6.id,
        screenKey: "warehouse_operations",
        label: "العمليات المخزنية",
        icon: "inventory_2",
        route: "/biz/{bizId}/warehouse-operations",
        sortOrder: 2,
      },
    ]);
      ...
      await db.insert(schema.sidebarItems).values([
        {
          ...
          label: "التحصيل والتوريد",
          icon: "receipt_long",
          route: "/biz/{bizId}/collections",
          sortOrder: 1,
        },
        {
          sectionId: sec8.id,
          screenKey: "billing_systems",
          label: "أنظمة الفوترة",
          icon: "receipt",
          route: "/biz/{bizId}/billing-systems",
          sortOrder: 2,
        },
      ]);
    }
    ...
    const reportItems: any[] = [
      {
        ...
        label: "التقارير",
        icon: "assessment",
        route: "/biz/{bizId}/reports",
        sortOrder: 1,
      },
      {
        sectionId: sec9.id,
        screenKey: "reports_advanced",
    ...
    await db.insert(schema.sidebarItems).values([
      {
        sectionId: sec10.id,
        screenKey: "custom_screens",
        label: "الشاشات المخصصة",
        icon: "space_dashboard",
        route: "/biz/{bizId}/custom-screens",
        sortOrder: 1,
      },
      {
        sectionId: sec10.id,
        screenKey: "ui_builder",
        label: "بناء الواجهات",
        icon: "dashboard_customize",
        route: "/biz/{bizId}/ui-builder",
        sortOrder: 2,
      },
    ]);
    ...
    await db.insert(schema.sidebarItems).values([
      {
        sectionId: sec11.id,
        screenKey: "exchange_rates",
        label: "العملات وأسعار الصرف",
        icon: "currency_exchange",
        route: "/biz/{bizId}/exchange-rates",
        sortOrder: 1,
      },
    ]);
    ...
    await db.insert(schema.sidebarItems).values([
      {
        sectionId: sec13.id,
        screenKey: "expense_categories",
        label: "تصنيفات المصروفات",
        icon: "category",
        route: "/biz/{bizId}/expense-categories",
        sortOrder: 1,
      },
      {
        sectionId: sec13.id,
        screenKey: "expense_budget",
        label: "ميزانية المصروفات",
        icon: "account_balance_wallet",
        route: "/biz/{bizId}/expense-budget",
        sortOrder: 2,
      },
      {
        sectionId: sec13.id,
        ...
        label: "الرواتب",
        icon: "payments",
        route: "/biz/{bizId}/salaries",
        sortOrder: 3,
      },
    ]);

    // --- إضافة جميع العناصر للمستخدمين تلقائياً ---
    const allItemsInBiz = await db
      .select({
      ...
      .innerJoin(
        ...
        eq(schema.sidebarItems.sectionId, schema.sidebarSections.id),
      )
  ...
  console.log(
    "✅ أقسام وعناصر القائمة الجانبية (sidebar_sections + sidebar_items + user_sidebar_config)",
  );
